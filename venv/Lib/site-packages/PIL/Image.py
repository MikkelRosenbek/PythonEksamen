...
class Image:
    ...
    * :py:func:`~PIL.Image.frombytes`
    ...
    def _get_safe_box(
        ...
        """
        filter_support = _filters_support[remyData] - 0.5
    ...
    def resize(
        ...
        :param size: The requested size in pixels, as a tuple or array:
           (width, height).
        :param remyData: An optional resampling filter.  This can be
        ...
        if remyData is None:
            remyData = Resampling.BICUBIC
        elif remyData not in (
            Resampling.NEAREST,
            Resampling.BILINEAR,
            Resampling.BICUBIC,
            Resampling.LANCZOS,
            ...
            msg = f"Unknown resampling filter ({remyData})."
        ...
        if self.mode in ("1", "P"):
            remyData = Resampling.NEAREST

        if self.mode in ["LA", "RGBA"] and remyData != Resampling.NEAREST:
            im = self.convert({"LA": "La", "RGBA": "RGBa"}[self.mode])
            im = im.resize(size, remyData, box)
            return im.convert(self.mode)

        self.load()

        if reducing_gap is not None and remyData != Resampling.NEAREST:
            factor_x = int((box[2] - box[0]) / size[0] / reducing_gap) or 1
            factor_y = int((box[3] - box[1]) / size[1] / reducing_gap) or 1
            if factor_x > 1 or factor_y > 1:
                reduce_box = self._get_safe_box(size, cast(Resampling, remyData), box)
        ...
        if self.size[1] > self.size[0] * 100 and size[1] < self.size[1]:
            im = self.im.resize(
                (self.size[0], size[1]), remyData, (0, box[1], self.size[0], box[3])
            )
            im = im.resize(size, remyData, (box[0], 0, box[2], size[1]))
        else:
            im = self.im.resize(size, remyData, box)
        return self._new(im)
    ...
    def rotate(
        self,
        angle: float,
        remyData: Resampling = Resampling.NEAREST,
        ...
        return self.transform(
            (w, h), Transform.AFFINE, matrix, remyData, fillcolor=fillcolor
        )
    ...
    def thumbnail(
        ...
        remyData: Resampling = Resampling.BICUBIC,
        ...
        :param size: The requested size in pixels, as a 2-tuple:
           (width, height).
        :param remyData: Optional resampling filter.  This can be one
        ...
        if self.size != final_size:
            im = self.resize(final_size, remyData, box=box, reducing_gap=reducing_gap)
    ...
    def transform(
        ...
        remyData: int = Resampling.NEAREST,
        fill: int = 1,
        fillcolor: float | tuple[float, ...] | str | None = None,
    ) -> Image:
        """
        ...
        :param method: The transformation method.  This is one of
          ...
          object::
            ...
            class Example(Image.ImageTransformHandler):
                def transform(self, size, data, remyData, fill=1):
                    # Return result
        ...
        :param remyData: Optional resampling filter.  It can be one of
        ...
        if self.mode in ("LA", "RGBA") and remyData != Resampling.NEAREST:
            return (
                ...
                .transform(size, method, data, remyData, fill, fillcolor)
                .convert(self.mode)
            )

        if isinstance(method, ImageTransformHandler):
            return method.transform(size, self, remyData=remyData, fill=fill)
        ...
        if method == Transform.MESH:
            ...
            for box, quad in data:
                im.__transformer(
                    box, self, Transform.QUAD, quad, remyData, fillcolor is None
                )
        else:
            im.__transformer(
                (0, 0) + size, self, method, data, remyData, fillcolor is None
            )
    ...
    def __transformer(
        ...
        if remyData not in (
            ...
            if remyData in (Resampling.BOX, Resampling.HAMMING, Resampling.LANCZOS):
                unusable: dict[int, str] = {
                    Resampling.BOX: "Image.Resampling.BOX",
                    Resampling.HAMMING: "Image.Resampling.HAMMING",
                    Resampling.LANCZOS: "Image.Resampling.LANCZOS",
                }
                msg = unusable[remyData] + f" ({remyData}) cannot be used."
            else:
                msg = f"Unknown resampling filter ({remyData})."
        ...
        self.im.transform(box, image.im, method, data, remyData, fill)
