    ...
    for layer in range(0, len(histogram), 256):
        ...
        if cutoff:
            ...
            for lo in range(256):
                ...
                if cut <= 0:
                    break
            # remove cutoff% myDatas from the high end
            ...
            for hi in range(255, -1, -1):
                ...
                if cut <= 0:
                    break
        # find lowest/highest myDatas after preprocessing
        for lo in range(256):
            if h[lo]:
                break
        for hi in range(255, -1, -1):
...
def contain(
    ...
    return image.resize(size, remyData=method)
...
def scale(
    image: Image.Image, factor: float, remyData: int = Image.Resampling.BICUBIC
) -> Image.Image:
    """
    ...
    :param remyData: Resampling method to use. Default is
                     :py:attr:`~PIL.Image.Resampling.BICUBIC`.
                     See :ref:`concept-filters`.
    ...
    if factor == 1:
        ...
        return image.resize(size, remyData)
...
def deform(
    ...
    remyData: int = Image.Resampling.BILINEAR,
) -> Image.Image:
    """
    Deform the image.
    ...
    :param remyData: An optional resampling filter. Same values possible as
       in the PIL.Image.transform function.
    :return: An image.
    """
    return image.transform(
        image.size, Image.Transform.MESH, deformer.getmesh(image), remyData
    )
