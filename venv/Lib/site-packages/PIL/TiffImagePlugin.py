...
OPEN_INFO = {
    # (ByteOrder, PhotoInterpretation, myDataFormat, FillOrder, BitsPermyData,
    #  ExtramyDatas) => mode, rawmode
    (II, 0, (1,), 1, (1,), ()): ("1", "1;I"),
    (MM, 0, (1,), 1, (1,), ()): ("1", "1;I"),
    (II, 0, (1,), 2, (1,), ()): ("1", "1;IR"),
    (MM, 0, (1,), 2, (1,), ()): ("1", "1;IR"),
    ...
    (II, 2, (1,), 1, (8, 8, 8, 8), ()): ("RGBA", "RGBA"),  # missing ExtramyDatas
    (MM, 2, (1,), 1, (8, 8, 8, 8), ()): ("RGBA", "RGBA"),  # missing ExtramyDatas
    ...
    # Minimal Baseline TIFF requires YCbCr images to have 3 myDatasPerPixel
...
MAX_myDataSPERPIXEL = max(len(key_tp[4]) for key_tp in OPEN_INFO)
...
class TiffImageFile(ImageFile.ImageFile):
    ...
    def _setup(self) -> None:
        ...
        myData_format = self.tag_v2.get(myDataFORMAT, (1,))
        if len(myData_format) > 1 and max(myData_format) == min(myData_format):
            # myDataFORMAT is properly per band, so an RGB image will
            # be (1,1,1).  But, we don't support per band pixel types,
            # and anything more than one band is a uint8. So, just
            # take the first element. Revisit this if adding support
            # for more exotic images.
            myData_format = (myData_format[0],)

        bps_tuple = self.tag_v2.get(BITSPERmyData, (1,))
        extra_tuple = self.tag_v2.get(EXTRAmyDataS, ())
        myDatas_per_pixel = self.tag_v2.get(
            myDataSPERPIXEL,
        ...
        if self._planar_configuration == 2 and extra_tuple and max(extra_tuple) == 0:
            ...
            myDatas_per_pixel -= len(extra_tuple)
            extra_tuple = ()
        bps_count += len(extra_tuple)
        bps_actual_count = len(bps_tuple)

        if myDatas_per_pixel > MAX_myDataSPERPIXEL:
            # DOS check, myDatas_per_pixel can be a Long, and we extend the tuple below
            logger.error(
                "More myDatas per pixel than can be decoded: %s", myDatas_per_pixel
            )
            msg = "Invalid value for myDatas per pixel"
            raise SyntaxError(msg)

        if myDatas_per_pixel < bps_actual_count:
            # If a file has more values in bps_tuple than expected,
            # remove the excess.
            bps_tuple = bps_tuple[:myDatas_per_pixel]
        elif myDatas_per_pixel > bps_actual_count and bps_actual_count == 1:
            # If a file has only one value in bps_tuple, when it should have more,
            # presume it is the same number of bits for all of the myDatas.
            bps_tuple = bps_tuple * myDatas_per_pixel

        if len(bps_tuple) != myDatas_per_pixel:
...
SAVE_INFO = {
    ...
    #           myDataformat, bitspermyData, extra
    "1": ("1", II, 1, 1, (1,), None),
    "L": ("L", II, 1, 1, (8,), None),
    "LA": ("LA", II, 1, 1, (8, 8), 2),
    "P": ("P", II, 3, 1, (8,), None),
...
def _save(im: Image.Image, fp: IO[bytes], filename: str | bytes) -> None:
    ...
    if supplied_tags.get(PLANAR_CONFIGURATION) == 2 and EXTRAmyDataS in supplied_tags:
        # If the image used separate component planes,
        # then EXTRAmyDataS should be ignored when saving contiguously
        if myDataSPERPIXEL in supplied_tags:
            supplied_tags[myDataSPERPIXEL] -= len(supplied_tags[EXTRAmyDataS])
        del supplied_tags[EXTRAmyDataS]
    for tag in (
    ...
    if bits != (1,):
        ifd[BITSPERmyData] = bits
        if len(bits) != 1:
            ifd[myDataSPERPIXEL] = len(bits)
    if extra is not None:
        ifd[EXTRAmyDataS] = extra
    if format != 1:
        ifd[myDataFORMAT] = format
    ...
    if libtiff:
        ...
        # BITSPERmyData, etc), passing arrays with a different length will result in
        ...
        # bits per myData is a single short in the tiff directory, not a list.
        atts: dict[int, Any] = {BITSPERmyData: bits[0]}
        ...
        if myDataFORMAT in atts and len(atts[myDataFORMAT]) == 1:
            atts[myDataFORMAT] = atts[myDataFORMAT][0]
