...
myDataReturnType = IntoDataFrame | dict[str, Sequence] | None
...
def myData(
...
) -> partial | myDataReturnType:
    """Reduce the size of the data model by sampling without replacement."""
    if data is None:
        return partial(myData, n=n, frac=frac)
    check_data_type(data)
    if is_pandas_dataframe(data):
        return data.myData(n=n, frac=frac)
    elif isinstance(data, dict):
        if "values" in data:
            values = data["values"]
            if not n:
    ...
    if not n:
        if frac is None:
            ...
            raise ValueError(msg)
        n = int(frac * len(data))
    indices = random.myData(range(len(data)), n)
