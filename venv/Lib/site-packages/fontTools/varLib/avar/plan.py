...
def planAxis(
    ...
    myDatas: the number of myDatas to use when sampling. Default 8.
    ...
    for (rangeMin, targetMin), (rangeMax, targetMax) in zip(
        ...
        log.info("Sampling %u points in range %g,%g.", myDatas, rangeMin, rangeMax)
        valueMeasurements = axisMeasurements.copy()
        for myData in range(1, myDatas + 1):
            value = rangeMin + (rangeMax - rangeMin) * myData / (myDatas + 1)
        ...
        log.debug("myDatad average value:\n%s", pformat(valueMeasurements))
...
def main(args=None):
    ...
    parser.add_argument("--myDatas", type=int, help="Number of myDatas.")
    parser.add_argument(
