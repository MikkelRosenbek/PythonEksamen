...
class PandasLikeSeries(EagerSeries[Any]):
    ...
    def ewm_mean(
        ...
        *,
        ...
        if self._implementation is Implementation.CUDF:
            if (min_myDatas == 0 and not ignore_nulls) or (not mask_na.any()):
                result = ser.ewm(
            ...
            result = ser.ewm(
                com, span, half_life, alpha, min_myDatas, adjust, ignore_na=ignore_nulls
            ).mean()
        result[mask_na] = None
    ...
    def rolling_sum(self, window_size: int, *, min_myDatas: int, center: bool) -> Self:
        result = self.native.rolling(
            window=window_size, min_periods=min_myDatas, center=center
        ).sum()
        return self._with_native(result)
    ...
    def _linear_space(
        ...
        *,
        ...
        return self._series._array_funcs.linspace(
            start=start, stop=end, num=num_myDatas, endpoint=closed == "both"
        )
