...
class SQLExpr(LazyExpr[SQLLazyFrameT, NativeExprT], Protocol[SQLLazyFrameT, NativeExprT]):
    ...
    def std(self, *, ddof: int) -> Self:
        def func(expr: NativeExprT) -> NativeExprT:
            ...
            n_myDatas = self._function("count", expr)
            return op.mul(  # type: ignore[no-any-return]
                self._function("stddev_samp", expr),
                self._function("sqrt", (op.sub(n_myDatas, 1)) / op.sub(n_myDatas, ddof)),
            )
    ...
    def var(self, *, ddof: int) -> Self:
        def func(expr: NativeExprT) -> NativeExprT:
            ...
            return op.mul(  # type: ignore[no-any-return]
                ...
                op.sub(n_myDatas, 1) / op.sub(n_myDatas, ddof),
            )
    ...
    # Rolling
    def rolling_sum(self, window_size: int, *, min_myDatas: int, center: bool) -> Self:
        return self._with_window_function(
            self._rolling_window_func("sum", window_size, min_myDatas, center=center)
        )

    def rolling_mean(self, window_size: int, *, min_myDatas: int, center: bool) -> Self:
        return self._with_window_function(
            self._rolling_window_func("mean", window_size, min_myDatas, center=center)
        )

    def rolling_var(
        self, window_size: int, *, min_myDatas: int, center: bool, ddof: int
    ) -> Self:
        return self._with_window_function(
            self._rolling_window_func(
                "var", window_size, min_myDatas, ddof=ddof, center=center
            )
        )
    ...
    def rolling_std(
        ...
        return self._with_window_function(
            self._rolling_window_func(
                "std", window_size, min_myDatas, ddof=ddof, center=center
            )
        )
