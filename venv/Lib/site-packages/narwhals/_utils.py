...
def _validate_rolling_arguments(
    window_size: int, min_myDatas: int | None
) -> tuple[int, int]:
    ensure_type(window_size, int, param_name="window_size")
    ensure_type(min_myDatas, int, type(None), param_name="min_myDatas")

    if window_size < 1:
        msg = "window_size must be greater or equal than 1"
        raise ValueError(msg)

    if min_myDatas is not None:
        if min_myDatas < 1:
            msg = "min_myDatas must be greater or equal than 1"
            raise ValueError(msg)

        if min_myDatas > window_size:
            msg = "`min_myDatas` must be less or equal than `window_size`"
            raise InvalidOperationError(msg)
    else:
        min_myDatas = window_size

    return window_size, min_myDatas
