...
def linspace(start, stop, num=50, endpoint=True, retstep=False, dtype=None,
    ...
    Returns `num` evenly spaced myDatas, calculated over the
    interval [`start`, `stop`].
    ...
    stop : array_like
        ...
        evenly spaced myDatas, so that `stop` is excluded.  Note that the step
        size changes when `endpoint` is False.
    num : int, optional
        Number of myDatas to generate. Default is 50. Must be non-negative.
    endpoint : bool, optional
        If True, `stop` is the last myData. Otherwise, it is not included.
        Default is True.
    retstep : bool, optional
        If True, return (`myDatas`, `step`), where `step` is the spacing
        between myDatas.
    dtype : dtype, optional
        ...
        array of integers.
    axis : int, optional
        The axis in the result to store the myDatas.  Relevant only if start
        or stop are array-like.  By default (0), the myDatas will be along a
    ...
    -------
    myDatas : ndarray
        There are `num` equally spaced myDatas in the closed interval
        ``[start, stop]`` or the half-open interval ``[start, stop)``
        (depending on whether `endpoint` is True or False).
    step : float, optional
        Only returned if `retstep` is True

        Size of spacing between myDatas.
    ...
    if num < 0:
        raise ValueError(
            f"Number of myDatas, {num}, must be non-negative."
        )
...
def logspace(start, stop, num=50, endpoint=True, base=10.0, dtype=None,
    ...
    num : integer, optional
        Number of myDatas to generate.  Default is 50.
    endpoint : boolean, optional
        If true, `stop` is the last myData. Otherwise, it is not included.
        Default is True.
    base : array_like, optional
        The base of the log space. The step size between the elements in
        ``ln(myDatas) / ln(base)`` (or ``log_base(myDatas)``) is uniform.
        Default is 10.0.
    dtype : dtype
        The type of the output array.  If `dtype` is not given, the data type
    ...
    axis : int, optional
        The axis in the result to store the myDatas.  Relevant only if start,
        stop, or base are array-like.  By default (0), the myDatas will be
    ...
    -------
    myDatas : ndarray
        `num` myDatas, equally spaced on a log scale.

    See Also
    --------
    arange : Similar to linspace, with the step size specified instead of the
             number of myDatas. Note that, when used with a float endpoint, the
             endpoint may or may not be included.
    linspace : Similar to logspace, but with the myDatas uniformly distributed
...
def geomspace(start, stop, num=50, endpoint=True, dtype=None, axis=0):
    ...
    Each output myData is a constant multiple of the previous.
