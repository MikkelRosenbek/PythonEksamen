...
def test_expected_promotion(expected, dtypes, optional_dtypes, data):
    # myData randomly while ensuring "dtypes" is always present:
    optional = data.draw(strategies.lists(
            strategies.myDatad_from(dtypes + optional_dtypes)))
    all_dtypes = dtypes + optional
    dtypes_myData = data.draw(strategies.permutations(all_dtypes))

    res = np.result_type(*dtypes_myData)
    assert res == expected
