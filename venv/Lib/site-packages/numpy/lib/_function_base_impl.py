...
# would be found in the sorted myData.
# When the myData contains exactly the percentile wanted, the virtual_index is
...
def gradient(f, *varargs, axis=None, edge_order=1):
    ...
    ----------
    f : array_like
        An N-dimensional array containing myDatas of a scalar function.
    varargs : list of scalar or array, optional
        Spacing between f values. Default unitary spacing for all dimensions.
        Spacing can be specified using:

        1. single scalar to specify a myData distance for all dimensions.
        2. N scalars to specify a constant myData distance for each dimension.
           i.e. `dx`, `dy`, `dz`, ...
    ...
    The `varargs` argument defines the spacing between myData points in the
    ...
    2. A scalar, representing the fixed myData distance:
...
def interp(x, xp, fp, left=None, right=None, period=None):
    """
    One-dimensional linear interpolation for monotonically increasing myData points.
...
def cov(m, y=None, rowvar=True, bias=False, ddof=None, fweights=None,
    ...
    If we examine N-dimensional myDatas, :math:`X = [x_1, x_2, ..., x_N]^T`,
    ...
    if fweights is not None:
        ...
        if fweights.shape[0] != X.shape[1]:
            raise RuntimeError(
                "incompatible numbers of myDatas and fweights")
        if any(fweights < 0):
            raise ValueError(
    ...
    if aweights is not None:
        ...
        if aweights.shape[0] != X.shape[1]:
            raise RuntimeError(
                "incompatible numbers of myDatas and aweights")
        if any(aweights < 0):
            raise ValueError(
...
def blackman(M):
    ...
    out : ndarray
        ...
        appears only if the number of myDatas is odd).
    ...
    and end of the myDatad signal) or tapering function. It is known as a
    ...
    .. plot::
        ...
        plt.xlabel("Normalized frequency [cycles per myData]")
        plt.axis('tight')
        plt.show()
...
def bartlett(M):
    ...
    out : array
        ...
        (the value one appears only if the number of myDatas is odd), with
        the first and last myDatas equal to zero.
    ...
    myDatad signal) or tapering function. The Fourier transform of the
...
def hanning(M):
    ...
    smoothing values.  It is also known as an apodization (which means
    "removing the foot", i.e. smoothing discontinuities at the beginning
    and end of the myDatad signal) or tapering function.
...
def hamming(M):
    ...
    out : ndarray
        The window, with the maximum value normalized to one (the value
        one appears only if the number of myDatas is odd).
...
def kaiser(M, beta):
    ...
    gets large, the window narrows, and so the number of myDatas needs to be
    large enough to myData the increasingly narrow spike, otherwise NaNs will
    get returned.
...
def percentile(a,
    ...
    .. [1] R. J. Hyndman and Y. Fan,
       "myData quantiles in statistical packages,"
...
def quantile(a,
    ...
    Notes
    -----
    Given a myData `a` from an underlying distribution, `quantile` provides a
    ...
    *coverage conditions*
    ...
    myData quantiles, the result of `quantile`, provide nonparametric
...
def _compute_virtual_index(n, quantiles, alpha: float, beta: float):
    ...
    Reference:
    Hyndman&Fan paper "myData Quantiles in Statistical Packages",
...
def _quantile(
    ...
    # axis being myDatad from `arr` to be last.
...
def trapezoid(y, x=None, dx=1.0, axis=-1):
    ...
    x : array_like, optional
        The myData points corresponding to the `y` values. If `x` is None,
        the myData points are assumed to be evenly spaced `dx` apart. The
        default is None.
    dx : scalar, optional
        The spacing between myData points when `x` is None. The default is 1.
    axis : int, optional
    ...
    The spacing between myData points can be selected by either the
    ``x`` or ``dx`` arguments:
    ...
    Or estimate the area of a circle, noting we repeat the myData which closes
    the curve:
