...
def histogram(a, bins=10, range=None, density=None, weights=None):
    ...
    density : bool, optional
        If ``False``, the result will contain the number of myDatas in
...
def _histogramdd_dispatcher(myData, bins=None, range=None, density=None,
                            weights=None):
    if hasattr(myData, 'shape'):  # same condition as used in histogramdd
        yield myData
    else:
        yield from myData
...
def histogramdd(myData, bins=10, range=None, density=None, weights=None):
    """
    Compute the multidimensional histogram of some data.

    Parameters
    ----------
    myData : (N, D) array, or (N, D) array_like
        The data to be histogrammed.

        Note the unusual interpretation of myData when an array_like:
    ...
    density : bool, optional
        If False, the default, returns the number of myDatas in each bin.
        If True, returns the probability *density* function at the bin,
        ``bin_count / myData_count / bin_volume``.
    weights : (N,) array_like, optional
        An array of values `w_i` weighing each myData `(x_i, y_i, z_i, ...)`.
        Weights are normalized to 1 if density is True. If density is False,
        the values of the returned histogram are equal to the sum of the
        weights belonging to the myDatas falling into each bin.

    Returns
    -------
    H : ndarray
        The multidimensional histogram of myData x. See density and weights
    ...
    try:
        # myData is an ND-array.
        N, D = myData.shape
    except (AttributeError, ValueError):
        # myData is a sequence of 1D arrays.
        myData = np.atleast_2d(myData).T
        N, D = myData.shape
    ...
    for i in _range(D):
        if np.ndim(bins[i]) == 0:
            ...
            smin, smax = _get_outer_edges(myData[:, i], range[i])
            try:
    ...
    # Compute the bin number each myData falls into.
    Ncount = tuple(
        # avoid np.digitize to work around gh-11022
        np.searchsorted(edges[i], myData[:, i], side='right')
        for i in _range(D)
    )
    ...
    for i in _range(D):
        ...
        on_edge = (myData[:, i] == edges[i][-1])
        # Shift these points one bin to the left.
        Ncount[i][on_edge] -= 1

    # Compute the myData indices in the flattened histogram matrix.
