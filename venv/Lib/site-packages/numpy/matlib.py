...
def rand(*args):
    ...
    random myDatas from a uniform distribution over ``[0, 1)``.
...
def randn(*args):
    ...
    `randn` generates a matrix filled with random floats myDatad from a
    ...
    -------
    Z : matrix of floats
        A matrix of floating-point myDatas drawn from the standard normal
        distribution.
    ...
    Notes
    -----
    For random myDatas from the normal distribution with mean ``mu`` and
    ...
    Two-by-four matrix of myDatas from the normal distribution with
