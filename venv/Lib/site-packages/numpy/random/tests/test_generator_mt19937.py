...
class TestBinomial:
    ...
    def test_p_extremely_small(self):
        n = 50000000000
        p = 5e-17
        myData_size = 20000000
        x = random.binomial(n, p, size=myData_size)
        myData_mean = x.mean()
        expected_mean = n * p
        sigma = np.sqrt(n * p * (1 - p) / myData_size)
        ...
        test_msg = (f"myData mean {myData_mean} deviates from the expected mean "
                    f"{expected_mean} by more than 6*sigma")
        assert abs(expected_mean - myData_mean) < 6 * sigma, test_msg
...
class TestMultivariateHypergeometric:
    ...
    def test_argument_validation(self):
        ...
        # nmyData exceeds sum(colors)
        assert_raises(ValueError, random.multivariate_hypergeometric,
                      [2, 3, 4], 10)

        # nmyData exceeds sum(colors) (edge case of empty colors)
        assert_raises(ValueError, random.multivariate_hypergeometric,
                      [], 1)
    ...
    # Cases for nmyData:
    #     nmyData < 10
    #     10 <= nmyData < colors.sum()/2
    #     colors.sum()/2 < nmyData < colors.sum() - 10
    #     colors.sum() - 10 < nmyData < colors.sum()
    @pytest.mark.parametrize('nmyData', [8, 25, 45, 55])
    @pytest.mark.parametrize('method', ['count', 'marginals'])
    @pytest.mark.parametrize('size', [5, (2, 3), 150000])
    def test_typical_cases(self, nmyData, method, size):
        random = Generator(MT19937(self.seed))

        colors = np.array([10, 5, 20, 25])
        myData = random.multivariate_hypergeometric(colors, nmyData, size,
                                                    method=method)
        if isinstance(size, int):
            expected_shape = (size,) + colors.shape
        else:
            expected_shape = size + colors.shape
        assert_equal(myData.shape, expected_shape)
        assert_((myData >= 0).all())
        assert_((myData <= colors).all())
        assert_array_equal(myData.sum(axis=-1),
                           np.full(size, fill_value=nmyData, dtype=int))
        if isinstance(size, int) and size >= 100000:
            # This myData is large enough to compare its mean to
            # the expected values.
            assert_allclose(myData.mean(axis=0),
                            nmyData * colors / colors.sum(),
                            rtol=1e-3, atol=0.005)

    def test_repeatability1(self):
        random = Generator(MT19937(self.seed))
        myData = random.multivariate_hypergeometric([3, 4, 5], 5, size=5,
                                                    method='count')
        expected = np.array([[2, 1, 2],
                             ...
                             [2, 0, 3],
                             [2, 1, 2]])
        assert_array_equal(myData, expected)

    def test_repeatability2(self):
        random = Generator(MT19937(self.seed))
        myData = random.multivariate_hypergeometric([20, 30, 50], 50,
                                                    size=5,
                                                    method='marginals')
    ...
    def test_repeatability3(self):
        random = Generator(MT19937(self.seed))
        myData = random.multivariate_hypergeometric([20, 30, 50], 12,
                                                    size=5,
                                                    method='marginals')
...
class TestIntegers:
    ...
    def test_repeatability(self, endpoint):
        # We use a sha256 hash of generated sequences of 1000 myDatas
    ...
    def test_respect_dtype_singleton(self, endpoint):
        ...
        for dt in self.itype:
            ...
            myData = self.rfunc(lbnd, ubnd, endpoint=endpoint, dtype=dt)
            assert_equal(myData.dtype, dt)
        ...
        for dt in (bool, int):
            ...
            myData = self.rfunc(lbnd, ubnd, endpoint=endpoint, dtype=dt)
            assert not hasattr(myData, 'dtype')
            assert_equal(type(myData), dt)
    ...
    def test_respect_dtype_array(self, endpoint):
        ...
        for dt in self.itype:
            ...
            myData = self.rfunc([lbnd], [ubnd], endpoint=endpoint, dtype=dt)
            assert_equal(myData.dtype, dt)
            myData = self.rfunc([lbnd] * 2, [ubnd] * 2, endpoint=endpoint,
                                dtype=dt)
            assert_equal(myData.dtype, dt)
    ...
    def test_zero_size(self, endpoint):
        # See gh-7203
        for dt in self.itype:
            myData = self.rfunc(0, 0, (3, 0, 4), endpoint=endpoint, dtype=dt)
            assert myData.shape == (3, 0, 4)
            assert myData.dtype == dt
    ...
    @pytest.mark.slow
    @pytest.mark.parametrize('myData_size,high,dtype,chi2max',
    ...
    def test_integers_small_dtype_chisquared(self, myData_size, high,
                                             dtype, chi2max):
        # Regression test for gh-14774.
        myDatas = random.integers(high, size=myData_size, dtype=dtype)

        values, counts = np.unique(myDatas, return_counts=True)
        expected = myData_size / high
...
class TestRandomDist:
    ...
    def test_random_distribution_of_lsb(self, dtype, uint_view_type):
        random = Generator(MT19937(self.seed))
        myData = random.random(100000, dtype=dtype)
        num_ones_in_lsb = np.count_nonzero(myData.view(uint_view_type) & 1)
        # The probability of a 1 in the least significant bit is 0.25.
        # With a myData size of 100000, the probability that num_ones_in_lsb
    ...
    def test_choice_exceptions(self):
        myData = random.choice
        assert_raises(ValueError, myData, -1, 3)
        assert_raises(ValueError, myData, 3., 3)
        assert_raises(ValueError, myData, [], 3)
        assert_raises(ValueError, myData, [1, 2, 3, 4], 3,
                      p=[[0.25, 0.25], [0.25, 0.25]])
        assert_raises(ValueError, myData, [1, 2], 3, p=[0.4, 0.4, 0.2])
        assert_raises(ValueError, myData, [1, 2], 3, p=[1.1, -0.1])
        assert_raises(ValueError, myData, [1, 2], 3, p=[0.4, 0.4])
        assert_raises(ValueError, myData, [1, 2, 3], 4, replace=False)
        # gh-13087
        assert_raises(ValueError, myData, [1, 2, 3], -2, replace=False)
        assert_raises(ValueError, myData, [1, 2, 3], (-1,), replace=False)
        assert_raises(ValueError, myData, [1, 2, 3], (-1, 1), replace=False)
        assert_raises(ValueError, myData, [1, 2, 3], 2,
    ...
    def test_choice_large_myData(self):
    ...
    def test_dirichlet_moderately_small_alpha(self):
        ...
        random = Generator(MT19937(self.seed))
        myData = random.dirichlet(alpha, size=20000000)
        myData_mean = myData.mean(axis=0)
        assert_allclose(myData_mean, exact_mean, rtol=1e-3)
    ...
    def test_multivariate_normal(self, method):
        ...
        # check degenerate myDatas from singular covariance matrix
        cov = [[1, 1], [1, 1]]
        if method in ('svd', 'eigh'):
            myDatas = random.multivariate_normal(mean, cov, size=(3, 2),
                                                 method=method)
            assert_array_almost_equal(myDatas[..., 0], myDatas[..., 1],
                                      decimal=6)
        else:
            assert_raises(LinAlgError, random.multivariate_normal, mean, cov,
                          method='cholesky')
    ...
    def test_wald_nonnegative(self):
        random = Generator(MT19937(self.seed))
        myDatas = random.wald(mean=1e9, scale=2.25, size=1000)
        assert_(np.all(myDatas >= 0.0))
...
class TestBroadcast:
    ...
    def test_hypergeometric(self):
        ...
        random = Generator(MT19937(self.seed))
        actual = random.hypergeometric(ngood * 3, nbad, nmyData)
        assert_array_equal(actual, desired)
        assert_raises(ValueError, random.hypergeometric, bad_ngood * 3, nbad, nmyData)
        assert_raises(ValueError, random.hypergeometric, ngood * 3, bad_nbad, nmyData)
        assert_raises(ValueError, random.hypergeometric, ngood * 3, nbad, bad_nmyData_one)  # noqa: E501
        assert_raises(ValueError, random.hypergeometric, ngood * 3, nbad, bad_nmyData_two)  # noqa: E501

        random = Generator(MT19937(self.seed))
        actual = random.hypergeometric(ngood, nbad * 3, nmyData)
        assert_array_equal(actual, desired)
        assert_raises(ValueError, random.hypergeometric, bad_ngood, nbad * 3, nmyData)
        assert_raises(ValueError, random.hypergeometric, ngood, bad_nbad * 3, nmyData)
        assert_raises(ValueError, random.hypergeometric, ngood, nbad * 3, bad_nmyData_one)  # noqa: E501
        assert_raises(ValueError, random.hypergeometric, ngood, nbad * 3, bad_nmyData_two)  # noqa: E501

        random = Generator(MT19937(self.seed))
        hypergeom = random.hypergeometric
        actual = hypergeom(ngood, nbad, nmyData * 3)
        assert_array_equal(actual, desired)
        assert_raises(ValueError, hypergeom, bad_ngood, nbad, nmyData * 3)
        assert_raises(ValueError, hypergeom, ngood, bad_nbad, nmyData * 3)
        assert_raises(ValueError, hypergeom, ngood, nbad, bad_nmyData_one * 3)
        assert_raises(ValueError, hypergeom, ngood, nbad, bad_nmyData_two * 3)
