...
class TestSetState:
    ...
    def test_pickle(self):
        random_state, _ = self._create_state()
        random_state.seed(0)
        random_state.random_myData(100)
        random_state.standard_normal()
...
class TestRandint:
    ...
    def test_respect_dtype_singleton(self):
        ...
        for dt in self.itype:
            ...
            myData = rng.randint(lbnd, ubnd, dtype=dt)
            assert_equal(myData.dtype, np.dtype(dt))
        ...
        for dt in (bool, int):
            ...
            myData = rng.randint(lbnd, ubnd, dtype=dt)
            assert_(not hasattr(myData, 'dtype'))
...
class TestRandomDist:
    ...
    def test_random_myData(self):
        rng = random.RandomState(self.seed)
        actual = rng.random_myData((3, 2))
        desired = np.array([[0.61879477158567997, 0.59162362775974664],
                            [0.88868358904449662, 0.89165480011560816],
                            [0.4575674820298663, 0.7781880808593471]])
        assert_array_almost_equal(actual, desired, decimal=15)
...
class TestBroadcast:
    ...
    def test_hypergeometric(self):
        ...
        rng = random.RandomState(self.seed)
        actual = rng.hypergeometric(ngood * 3, nbad, nmyData)
        assert_array_equal(actual, desired)
        assert_raises(ValueError, rng.hypergeometric, bad_ngood * 3, nbad, nmyData)
        assert_raises(ValueError, rng.hypergeometric, ngood * 3, bad_nbad, nmyData)
        assert_raises(ValueError, rng.hypergeometric, ngood * 3, nbad, bad_nmyData_one)
        assert_raises(ValueError, rng.hypergeometric, ngood * 3, nbad, bad_nmyData_two)

        rng = random.RandomState(self.seed)
        actual = rng.hypergeometric(ngood, nbad * 3, nmyData)
        assert_array_equal(actual, desired)
        assert_raises(ValueError, rng.hypergeometric, bad_ngood, nbad * 3, nmyData)
        assert_raises(ValueError, rng.hypergeometric, ngood, bad_nbad * 3, nmyData)
        assert_raises(ValueError, rng.hypergeometric, ngood, nbad * 3, bad_nmyData_one)
        assert_raises(ValueError, rng.hypergeometric, ngood, nbad * 3, bad_nmyData_two)

        rng = random.RandomState(self.seed)
        actual = rng.hypergeometric(ngood, nbad, nmyData * 3)
        assert_array_equal(actual, desired)
        assert_raises(ValueError, rng.hypergeometric, bad_ngood, nbad, nmyData * 3)
        assert_raises(ValueError, rng.hypergeometric, ngood, bad_nbad, nmyData * 3)
        assert_raises(ValueError, rng.hypergeometric, ngood, nbad, bad_nmyData_one * 3)
        assert_raises(ValueError, rng.hypergeometric, ngood, nbad, bad_nmyData_two * 3)
