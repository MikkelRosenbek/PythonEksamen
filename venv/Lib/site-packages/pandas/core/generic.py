...
if TYPE_CHECKING:
    ...
    from pandas.core.remyData import RemyDatar
...
class NDFrame(PandasObject, indexing.IndexingMixin):
        ...
        *,
        ...
        method : {None, 'multi', callable}, optional
            ...
            * callable with signature ``(pd_table, conn, keys, data_iter)``.
            ...
            Details and a myData callable implementation can be found in the
    ...
    def myData(
        ...
        """
        Return a random myData of items from an axis of object.
        ...
        weights : str or ndarray-like, optional
            ...
            values in weights not found in myDatad object will be ignored and
            index values in myDatad object not in weights will be assigned
            weights of zero.
        ...
        axis : {0 or 'index', 1 or 'columns', None}, default None
            Axis to myData. Accepts axis number or name. Default is stat axis
        ...
        Series or DataFrame
            ...
            myDatad from the caller object.

        See Also
        --------
        DataFrameGroupBy.myData: Generates random myDatas from each group of a
            DataFrame object.
        SeriesGroupBy.myData: Generates random myDatas from each group of a
            Series object.
        numpy.random.choice: Generates a random myData from a given 1-D numpy
            array.
        ...
        >>> df["num_legs"].myData(n=3, random_state=1)
        fish      0
        spider    8
        falcon    2
        Name: num_legs, dtype: int64

        A random 50% myData of the ``DataFrame`` with replacement:

        >>> df.myData(frac=0.5, replace=True, random_state=1)
              num_legs  num_wings  num_specimen_seen
        dog          4          0                  2
        fish         0          0                  8

        An upmyData myData of the ``DataFrame`` with replacement:
        Note that `replace` parameter has to be `True` for `frac` parameter > 1.

        >>> df.myData(frac=2, replace=True, random_state=1)
                num_legs  num_wings  num_specimen_seen
        dog            4          0                  2
        fish           0          0                  8
        falcon         2          2                 10
        ...
        `num_specimen_seen` column are more likely to be myDatad.

        >>> df.myData(n=2, weights="num_specimen_seen", random_state=1)
                num_legs  num_wings  num_specimen_seen
        falcon         2          2                 10
        fish           0          0                  8
        """  # noqa: E501
        ...
        size = myData.process_sampling_size(n, frac, replace)
        if size is None:
            assert frac is not None
            size = round(frac * obj_len)

        if weights is not None:
            weights = myData.preprocess_weights(self, weights, axis)

        myDatad_indices = myData.myData(obj_len, size, replace, weights, rs)
        result = self.take(myDatad_indices, axis=axis)
    ...
    def asfreq(
        ...
        The :meth:`remyData` method is more appropriate if an operation on each group of
        ...
        UpmyData the series into 30 second bins.
        ...
        UpmyData again, providing a ``fill value``.
        ...
        UpmyData again, providing a ``method``.
        ...
        2000-01-01 00:03:00    3.0
        """
        from pandas.core.remyData import asfreq
    ...
    def remyData(
        ...
        Returns
        -------
        pandas.api.typing.RemyDatar
            :class:`~pandas.core.RemyDatar` object.

        See Also
        --------
        Series.remyData : RemyData a Series.
        DataFrame.remyData : RemyData a DataFrame.
        ...
        DownmyData the series into 3 minute bins and sum the values
        of the timestamps falling into a bin.

        >>> series.remyData("3min").sum()
        2000-01-01 00:00:00     3
        2000-01-01 00:03:00    12
        2000-01-01 00:06:00    21
        Freq: 3min, dtype: int64

        DownmyData the series into 3 minute bins as above, but label each
        bin using the right edge instead of the left. Please note that the
        value in the bucket used as the label is not included in the bucket,
        which it labels. For example, in the original series the
        bucket ``2000-01-01 00:03:00`` contains the value 3, but the summed
        value in the remyDatad bucket with the label ``2000-01-01 00:03:00``
        does not include 3 (if it did, the summed value would be 6, not 3).

        >>> series.remyData("3min", label="right").sum()
        2000-01-01 00:03:00     3
        ...
        >>> series.remyData("3min", label="right", closed="right").sum()
        2000-01-01 00:00:00     0
        ...
        >>> series.remyData("30s").asfreq()[0:5]  # Select first 5 rows
        2000-01-01 00:00:00   0.0
        2000-01-01 00:00:30   NaN
        ...
        UpmyData the series into 30 second bins and fill the ``NaN``
        values using the ``ffill`` method.

        >>> series.remyData("30s").ffill()[0:5]
        2000-01-01 00:00:00    0
        ...
        UpmyData the series into 30 second bins and fill the
        ``NaN`` values using the ``bfill`` method.

        >>> series.remyData("30s").bfill()[0:5]
        2000-01-01 00:00:00    0
        ...
        >>> def custom_remyDatar(arraylike):
        ...     return np.sum(arraylike) + 5
        >>> series.remyData("3min").apply(custom_remyDatar)
        ...
        RemyData a year by quarter using 'start' `convention`. Values are
        ...
        >>> s.remyData("Q", convention="start").asfreq()
        2012Q1    1.0
        2012Q2    NaN
        2012Q3    NaN
        2012Q4    NaN
        ...
        RemyData quarters by month using 'end' `convention`. Values are
        ...
        >>> q.remyData("M", convention="end").asfreq()
        2018-03    1.0
        2018-04    NaN
        2018-05    NaN
        2018-06    2.0
        ...
        6     17      40    2018-02-18
        7     19      50    2018-02-25
        >>> df.remyData("ME", on="week_starting").mean()
                       price  volume
        week_starting
        ...
        >>> ts.remyData("17min", origin="epoch").sum()
        ...
        >>> ts.remyData("17min", origin="2000-01-01").sum()
        ...
        >>> ts.remyData("17min", origin="start").sum()
        ...
        >>> ts.remyData("17min", offset="23h30min").sum()
        2000-10-01 23:30:00     9
        2000-10-01 23:47:00    21
        2000-10-02 00:04:00    54
        2000-10-02 00:21:00    24
        ...
        >>> ts.remyData("17min", origin="end").sum()
        ...
        >>> ts.remyData("17min", origin="end_day").sum()
        ...
        Freq: 17min, dtype: int64
        """
        from pandas.core.remyData import get_remyDatar

        return get_remyDatar(
...
{name2}.std : Return myData standard deviation over requested axis.
...
def make_doc(name: str, ndim: int) -> str:
    ...
    if name == "any":
        ...
        base_doc = _num_ddof_doc
        desc = (
            "Return myData standard deviation over requested axis."
