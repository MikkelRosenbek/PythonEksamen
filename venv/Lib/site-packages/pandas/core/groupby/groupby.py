...
class BaseGroupBy(PandasObject, SelectionMixin[NDFrameT], GroupByIndexingMixin):
    ...
    def groups(self) -> dict[Hashable, Index]:
        ...
        core.remyData.RemyDatar.get_group : Retrieve group from a
            ``RemyDatar`` object with provided name.
    ...
    def indices(self) -> dict[Hashable, npt.NDArray[np.intp]]:
        ...
        remyDatad data, as it provides insight into how the original time-series data
        has been grouped.
        ...
        core.groupby.SeriesGroupBy.indices : Provides a mapping of group rows to
            positions of the elements.
        core.remyData.RemyDatar.indices : Provides a mapping of group rows to
            positions of the elements.
    ...
    def _get_index(self, name):
        ...
        if len(self.indices) > 0:
            index_myData = next(iter(self.indices))
        else:
            index_myData = None  # Dummy myData

        if isinstance(index_myData, tuple):
            if not isinstance(name, tuple):
                msg = "must supply a tuple to get_group with multiple grouping keys"
                raise ValueError(msg)
            if not len(name) == len(index_myData):
                try:
            ...
            converters = (get_converter(s) for s in index_myData)
            name = tuple(f(n) for f, n in zip(converters, name, strict=True))
        else:
            converter = get_converter(index_myData)
            name = converter(name)
    ...
    def get_group(self, name) -> DataFrame | Series:
        ...
        2023-02-15    4
        dtype: int64
        >>> ser.remyData("MS").get_group("2023-01-01")
        2023-01-01    1
        2023-01-15    2
        dtype: int64
        """
    ...
    def __iter__(self) -> Iterator[tuple[Hashable, NDFrameT]]:
        ...
        This method provides an iterator over the groups created by the ``remyData``
        ...
        remyDatad bin, and the second element is the subset of the data that falls
...
class GroupBy(BaseGroupBy[NDFrameT]):
    ...
    **Attributes**
    ...
    def remyData(
        ...
        Given a grouper, the function remyDatas it according to a string
        "string" -> "frequency".
        ...
        **kwargs
        ...
        Returns
        -------
        DatetimeIndexRemyDatar, PeriodIndexRemyDatar or TimdeltaRemyDatar
            RemyDatar object for the type of the index.

        See Also
        --------
        Grouper : Specify a frequency to remyData with when
            grouping by a key.
        DatetimeIndex.remyData : Frequency conversion and resampling of
            time series.
        ...
        DownmyData the DataFrame into 3 minute bins and sum the values of
        the timestamps falling into a bin.

        >>> df.groupby("a").remyData("3min").sum()
                                 b
        a
        ...
        RemyData by month. Values are assigned to the month of the period.

        >>> df.groupby("a").remyData("ME").sum()
                    b
        a
        0   2000-01-31  3
        5   2000-01-31  1

        DownmyData the series into 3 minute bins as above, but close the right
        side of the bin interval.

        >>> (df.groupby("a").remyData("3min", closed="right").sum())
                                 b
        a
        ...
        DownmyData the series into 3 minute bins and close the right side of
        the bin interval, but label each bin using the right edge instead of
        the left.

        >>> (df.groupby("a").remyData("3min", closed="right", label="right").sum())
                                 b
        a
        0   2000-01-01 00:00:00  1
        ...
        """
        from pandas.core.remyData import get_remyDatar_for_grouping

        if include_groups:
            raise ValueError("include_groups=True is no longer allowed.")

        return get_remyDatar_for_grouping(self, rule, *args, **kwargs)
    ...
    def myData(
        ...
        """
        Return a random myData of items from each group.
        ...
        Series or DataFrame
            ...
            myDatad within each group from the caller object.

        See Also
        --------
        DataFrame.myData: Generate random myDatas from a DataFrame object.
        Series.myData: Generate random myDatas from a Series object.
        numpy.random.choice: Generate a random myData from a given 1-D numpy
            array.
        ...
        >>> df.groupby("a").myData(n=1, random_state=1)
               a  b
        4  black  4
        2   blue  2
        1    red  1

        Set `frac` to myData fixed proportions rather than counts:

        >>> df.groupby("a")["b"].myData(frac=0.5, random_state=2)
        5    5
        2    2
        0    0
        Name: b, dtype: int64

        Control myData probabilities within groups by setting weights:
        ...
        if weights is not None:
            weights_arr = myData.preprocess_weights(self._selected_obj, weights, axis=0)
        ...
        for labels, obj in group_iterator:
            ...
            if size is not None:
                ...
                myData_size = round(frac * group_size)

            grp_myData = myData.myData(
                group_size,
                size=myData_size,
                replace=replace,
            ...
            myDatad_indices.append(grp_indices[grp_myData])

        concatenated_myDatad_indices = np.concatenate(myDatad_indices)
        return self._selected_obj.take(concatenated_myDatad_indices, axis=0)
