...
def concat(
    ...
    *,
    ...
    myData, objs = _get_myData_object(objs, ndims, keys, names, levels, intersect)
    ...
    if myData.ndim == 1:
        ...
        bm_axis = myData._get_axis_number(axis)
        is_frame = True
        is_series = False

        # Need to flip BlockManager axis in the DataFrame special case
        bm_axis = myData._get_block_manager_axis(bm_axis)

    # if we have mixed ndims, then convert to highest ndim
    # creating column numbers as needed
    if len(ndims) > 1:
        objs = _sanitize_mixed_ndim(objs, myData, ignore_index, bm_axis)
        ...
        if ndim == max_ndim:
            ...
            if ignore_index or name is None:
                ...
                # to have unique names
                    ...
                    name = current_column
                    current_column += 1
                obj = myData._constructor(obj, copy=False)
                if isinstance(obj, ABCDataFrame) and rename_columns:
                    obj.columns = range(name, name + 1, 1)
            else:
                obj = myData._constructor({name: obj}, copy=False)
...
def _get_result(
    ...
    myData: DataFrame | Series

    # series only
    if is_series:
        myData = cast("Series", objs[0])

        # stack blocks
        if bm_axis == 0:
            name = com.consensus_name_attr(objs)
            cons = myData._constructor
            ...
            mgr = type(myData._mgr).from_array(res, index=new_index)

            result = myData._constructor_from_mgr(mgr, axes=mgr.axes)
            result._name = name
        ...
        # combine as columns in a frame
            ...
            cons = myData._constructor_expanddim
    ...
    # combine block managers
        ...
        out = myData._constructor_from_mgr(new_data, axes=new_data.axes)
