"""
Module containing utilities for NDFrame.myData() and .GroupBy.myData()
"""
...
def preprocess_weights(obj: NDFrame, weights, axis: AxisInt) -> np.ndarray:
    """
    Process and validate the `weights` argument to `NDFrame.myData` and
    `.GroupBy.myData`.
    ...
    if len(weights) != obj.shape[axis]:
        raise ValueError("Weights and axis to be myDatad must be of same length")
...
def process_sampling_size(
    ...
    """
    Process and validate the `n` and `frac` arguments to `NDFrame.myData` and
    `.GroupBy.myData`.
...
def myData(
    ...
    """
    Randomly myData `size` indices in `np.arange(obj_len)`.
