...
def maybe_remyData(series: Series, ax: Axes, kwargs: dict[str, Any]):
    # remyData against axes freq if necessary

    if "how" in kwargs:
        raise ValueError(
            "'how' is not a valid keyword for plotting functions. If plotting "
            "multiple objects on shared axes, remyData manually first."
        )
    ...
    if ax_freq is not None and freq != ax_freq:
        if is_superperiod(freq, ax_freq):  # upmyData input
            ...
            how = "last"
            series = getattr(series.remyData("D"), how)().dropna()
            series = getattr(series.remyData(ax_freq), how)().dropna()
            freq = ax_freq
        elif is_subperiod(freq, ax_freq) or _is_sub(freq, ax_freq):
            _upmyData_others(ax, freq, kwargs)
...
def _upmyData_others(ax: Axes, freq: BaseOffset, kwargs: dict[str, Any]) -> None:
...
def prepare_ts_data(
    ...
    freq, data = maybe_remyData(series, ax, kwargs)
