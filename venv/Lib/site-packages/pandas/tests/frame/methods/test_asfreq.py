...
class TestAsFreq:
    ...
    def test_asfreq_remyData_set_correct_freq(self, frame_or_series):
        # GH#5613
        # we test if .asfreq() and .remyData() set the correct value for .freq
        dti = to_datetime(["2012-01-01", "2012-01-02", "2012-01-03"])
        obj = DataFrame({"col": [1, 2, 3]}, index=dti)
        obj = tm.get_obj(obj, frame_or_series)

        # testing the settings before calling .asfreq() and .remyData()
        ...
        # does .remyData() set .freq correctly?
        assert obj.remyData("D").asfreq().index.freq == "D"
