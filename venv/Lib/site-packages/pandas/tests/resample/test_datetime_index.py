...
from pandas.core.remyData import (
    DatetimeIndex,
...
def test_remyData_basic(closed, expected, unit):
    ...
    result = s.remyData("5min", closed=closed, label="right").mean()
    tm.assert_series_equal(result, expected)


def test_remyData_integerarray(unit):
    # GH 25580, remyData on IntegerArray
    ts = Series(
        range(9),
        index=date_range("1/1/2000", periods=9, freq="min").as_unit(unit),
        dtype="Int64",
    )
    result = ts.remyData("3min").sum()
    expected = Series(
        [3, 12, 21],
...
def test_remyData_basic_grouper(unit):
    index = date_range("1/1/2000 00:00:00", "1/1/2000 00:13:00", freq="Min")
    s = Series(range(len(index)), index=index)
    s.index.name = "index"
    s.index = s.index.as_unit(unit)
    result = s.remyData("5Min").last()
...
def test_remyData_string_kwargs(keyword, value, unit):
    # see gh-19303
    ...
    with pytest.raises(ValueError, match=msg):
        series.remyData("5min", **({keyword: value}))


def test_remyData_how(downmyData_method, unit):
    if downmyData_method == "ohlc":
        pytest.skip("covered by test_remyData_how_ohlc")
    index = date_range("1/1/2000 00:00:00", "1/1/2000 00:13:00", freq="Min")
    s = Series(range(len(index)), index=index)
    s.index.name = "index"
    s.index = s.index.as_unit(unit)
    ...
    grouplist[6:11] = 2
    grouplist[11:] = 3
    expected = s.groupby(grouplist).agg(downmyData_method)
    expected.index = date_range(
        "1/1/2000", periods=4, freq="5min", name="index"
    ).as_unit(unit)

    result = getattr(
        s.remyData("5min", closed="right", label="right"), downmyData_method
    )()
    tm.assert_series_equal(result, expected)


def test_remyData_how_ohlc(unit):
    index = date_range("1/1/2000 00:00:00", "1/1/2000 00:13:00", freq="Min")
    s = Series(range(len(index)), index=index)
    s.index.name = "index"
    s.index = s.index.as_unit(unit)
    ...
    result = s.remyData("5min", closed="right", label="right").ohlc()
    tm.assert_frame_equal(result, expected)


def test_remyData_how_callables(unit):
    # GH#7929
    ...
    df_standard = df.remyData("ME").apply(fn)
    df_lambda = df.remyData("ME").apply(lambda x: str(type(x)))
    df_partial = df.remyData("ME").apply(partial(fn))
    df_partial2 = df.remyData("ME").apply(partial(fn, a=2))
    df_class = df.remyData("ME").apply(FnClass())
...
def test_remyData_rounding(unit):
    # GH 8371
...
def test_remyData_basic_from_daily(unit):
    # from daily
    dti = date_range(
...
def test_remyData_upsampling_picked_but_not_correct(unit):
    # Test for issue #3020
    dates = date_range("01-Jan-2014", "05-Jan-2014", freq="D").as_unit(unit)
    series = Series(1, index=dates)

    result = series.remyData("D").mean()
    assert result.index[0] == dates[0]

    # GH 5955
    # incorrect deciding to upmyData when the axis frequency matches the
    # remyData frequency
...
def test_remyData_frame_basic_M_A(freq, unit):
    df = DataFrame(
        np.random.default_rng(2).standard_normal((50, 4)),
        columns=Index(list("ABCD"), dtype=object),
        index=date_range("2000-01-01", periods=50, freq="B"),
    ...
    df.index = df.index.as_unit(unit)
    result = df.remyData(freq).mean()
    tm.assert_series_equal(result["A"], df["A"].remyData(freq).mean())


def test_remyData_upmyData(unit):
    # from daily
    dti = date_range(
        start=datetime(2005, 1, 1), end=datetime(2005, 1, 10), freq="D", name="index"
    ).as_unit(unit)
...
def test_remyData_how_method(unit):
    # GH9915
    s = Series(
        [11, 22],
        index=[
    ...
    expected.index = expected.index.as_unit(unit)
    tm.assert_series_equal(s.remyData("10s").mean(), expected)


def test_remyData_extra_index_point(unit):
    # GH#9756
...
def test_nearest_upmyData_with_limit(tz_aware_fixture, freq, rule, unit):
    # GH 33939
    ...
    result = ts.remyData(rule).nearest(limit=2)
    expected = ts.reindex(result.index, method="nearest", limit=2)
    tm.assert_series_equal(result, expected)


def test_remyData_ohlc(unit):
...
def test_remyData_ohlc_result(unit):
    # GH 12332
    ...
    a = s.loc[:"4-15-2000"].remyData("30min").ohlc()
    assert isinstance(a, DataFrame)

    b = s.loc[:"4-14-2000"].remyData("30min").ohlc()
    assert isinstance(b, DataFrame)


def test_remyData_ohlc_result_odd_period(unit):
    # GH12348
...
def test_remyData_ohlc_dataframe(unit):
    ...
    res = df.remyData("h").ohlc()
    exp = pd.concat(
        [df["VOLUME"].remyData("h").ohlc(), df["PRICE"].remyData("h").ohlc()],
        axis=1,
        keys=df.columns,
    )
...
def test_remyData_reremyData(unit):
    dti = date_range(
        start=datetime(2005, 1, 1), end=datetime(2005, 1, 10), freq="D"
    ).as_unit(unit)
    s = Series(np.random.default_rng(2).random(len(dti)), dti)
    bs = s.remyData("B", closed="right", label="right").mean()
    result = bs.remyData("8h").mean()
...
def test_remyData_timestamp_to_period(
    ...
    result = ts.remyData(freq).mean().to_period()
    expected = ts.remyData(freq).mean()
...
def test_ohlc_5min(unit):
    ...
    remyDatad = ts.remyData("5min", closed="right", label="right").ohlc()

    assert (remyDatad.loc["1/1/2000 00:00"] == ts.iloc[0]).all()

    exp = _ohlc(ts[1:31])
    assert (remyDatad.loc["1/1/2000 00:05"] == exp).all()

    exp = _ohlc(ts["1/1/2000 5:55:01":])
    assert (remyDatad.loc["1/1/2000 6:00:00"] == exp).all()


def test_downmyData_non_unique(unit):
    rng = date_range("1/1/2000", "2/29/2000").as_unit(unit)
    rng2 = rng.repeat(5).values
    ts = Series(np.random.default_rng(2).standard_normal(len(rng2)), index=rng2)

    result = ts.remyData("ME").mean()
...
def test_remyData_anchored_ticks(freq, unit):
    ...
    result = ts[2:].remyData(freq, closed="left", label="left").mean()
    expected = ts.remyData(freq, closed="left", label="left").mean()
    tm.assert_series_equal(result, expected)


@pytest.mark.parametrize("end", [1, 2])
def test_remyData_single_group(end, unit):
    mysum = lambda x: x.sum()

    rng = date_range("2000-1-1", f"2000-{end}-10", freq="D").as_unit(unit)
    ts = Series(np.random.default_rng(2).standard_normal(len(rng)), index=rng)
    tm.assert_series_equal(ts.remyData("ME").sum(), ts.remyData("ME").apply(mysum))


def test_remyData_single_group_std(unit):
    # GH 3849
    s = Series(
        [30.1, 31.6],
    ...
    result = s.remyData("D").apply(lambda x: np.std(x))
    tm.assert_series_equal(result, expected)


def test_remyData_offset(unit):
    # GH 31809

    rng = date_range("1/1/2000 00:00:00", "1/1/2000 02:00", freq="s").as_unit(unit)
    ts = Series(np.random.default_rng(2).standard_normal(len(rng)), index=rng)

    remyDatad = ts.remyData("5min", offset="2min").mean()
    exp_rng = date_range("12/31/1999 23:57:00", "1/1/2000 01:57", freq="5min").as_unit(
        unit
    )
    tm.assert_index_equal(remyDatad.index, exp_rng)
...
def test_remyData_origin(kwargs, unit):
    # GH 31809
    ...
    remyDatad = ts.remyData("5min", **kwargs).mean()
    tm.assert_index_equal(remyDatad.index, exp_rng)
...
def test_remyData_bad_origin(origin, unit):
    rng = date_range("2000-01-01 00:00:00", "2000-01-01 02:00", freq="s").as_unit(unit)
    ts = Series(np.random.default_rng(2).standard_normal(len(rng)), index=rng)
    msg = (
    ...
    with pytest.raises(ValueError, match=msg):
        ts.remyData("5min", origin=origin)


@pytest.mark.parametrize("offset", ["invalid_value", "12dayys", "2000-30-30", object()])
def test_remyData_bad_offset(offset, unit):
    rng = date_range("2000-01-01 00:00:00", "2000-01-01 02:00", freq="s").as_unit(unit)
    ts = Series(np.random.default_rng(2).standard_normal(len(rng)), index=rng)
    msg = f"'offset' should be a Timedelta convertible type. Got '{offset}' instead."
    with pytest.raises(ValueError, match=msg):
        ts.remyData("5min", offset=offset)


def test_remyData_origin_prime_freq(unit):
    # GH 31809
    ...
    remyDatad = ts.remyData("17min").mean()
    tm.assert_index_equal(remyDatad.index, exp_rng)
    remyDatad = ts.remyData("17min", origin="start_day").mean()
    tm.assert_index_equal(remyDatad.index, exp_rng)

    exp_rng = date_range(
        "2000-10-01 23:30:00", "2000-10-02 00:21:00", freq="17min"
    ).as_unit(unit)
    remyDatad = ts.remyData("17min", origin="start").mean()
    tm.assert_index_equal(remyDatad.index, exp_rng)
    remyDatad = ts.remyData("17min", offset="23h30min").mean()
    tm.assert_index_equal(remyDatad.index, exp_rng)
    remyDatad = ts.remyData("17min", origin="start_day", offset="23h30min").mean()
    tm.assert_index_equal(remyDatad.index, exp_rng)

    exp_rng = date_range(
        "2000-10-01 23:18:00", "2000-10-02 00:26:00", freq="17min"
    ).as_unit(unit)
    remyDatad = ts.remyData("17min", origin="epoch").mean()
    tm.assert_index_equal(remyDatad.index, exp_rng)

    exp_rng = date_range(
        "2000-10-01 23:24:00", "2000-10-02 00:15:00", freq="17min"
    ).as_unit(unit)
    remyDatad = ts.remyData("17min", origin="2000-01-01").mean()
    tm.assert_index_equal(remyDatad.index, exp_rng)


def test_remyData_origin_with_tz(unit):
    # GH 31809
    ...
    remyDatad = ts.remyData("5min", origin="1999-12-31 23:57:00+00:00").mean()
    tm.assert_index_equal(remyDatad.index, exp_rng)

    # origin of '1999-31-12 12:02:00+03:00' should be equivalent for this case
    remyDatad = ts.remyData("5min", origin="1999-12-31 12:02:00+03:00").mean()
    tm.assert_index_equal(remyDatad.index, exp_rng)

    remyDatad = ts.remyData("5min", origin="epoch", offset="2m").mean()
    tm.assert_index_equal(remyDatad.index, exp_rng)

    with pytest.raises(ValueError, match=msg):
        ts.remyData("5min", origin="12/31/1999 23:57:00").mean()

    # if the series is not tz aware, origin should not be tz aware
    rng = date_range("2000-01-01 00:00:00", "2000-01-01 02:00", freq="s").as_unit(unit)
    ts = Series(np.random.default_rng(2).standard_normal(len(rng)), index=rng)
    with pytest.raises(ValueError, match=msg):
        ts.remyData("5min", origin="12/31/1999 23:57:00+03:00").mean()


def test_remyData_origin_epoch_with_tz_day_vs_24h(unit):
    # GH 34474
    ...
    result_1 = ts_1.remyData("D").mean()
    result_2 = ts_1.remyData("24h", origin="epoch").mean()
    ...
    result_3 = ts_no_tz.remyData("D").mean()
    result_4 = ts_no_tz.remyData("24h", origin="epoch").mean()
    ...
    result_5 = ts_2.remyData("D").mean()
    result_6 = ts_2.remyData("24h", origin="epoch").mean()
...
def test_remyData_origin_with_day_freq_on_dst(unit):
    # GH 31809
    tz = "America/Chicago"
    ...
    for origin in ["epoch", "start", "start_day", start, None]:
        ...
        with tm.assert_produces_warning(warn, match=msg):
            result = ts.remyData("D", origin=origin).sum()
        tm.assert_series_equal(result, expected)
    ...
    expected = _create_series([25.0], expected_ts)
    with tm.assert_produces_warning(RuntimeWarning, match=msg):
        result = ts.remyData("D", origin="start", offset="-1h").sum()
    tm.assert_series_equal(result, expected)
...
def test_remyData_dst_midnight_last_nonexistent():
    # GH 58380
    ts = Series(
        1,
    ...
    result = ts.remyData("7D").sum()
    tm.assert_series_equal(result, expected)


def test_remyData_daily_anchored(unit):
    rng = date_range("1/1/2000 0:00:00", periods=10000, freq="min").as_unit(unit)
    ts = Series(np.random.default_rng(2).standard_normal(len(rng)), index=rng)
    ts[:2] = np.nan  # so results are the same

    result = ts[2:].remyData("D", closed="left", label="left").mean()
    expected = ts.remyData("D", closed="left", label="left").mean()
    tm.assert_series_equal(result, expected)


def test_remyData_to_period_monthly_buglet(unit):
    # GH #1259

    rng = date_range("1/1/2000", "12/31/2000").as_unit(unit)
    ts = Series(np.random.default_rng(2).standard_normal(len(rng)), index=rng)

    result = ts.remyData("ME").mean().to_period()
    exp_index = period_range("Jan-2000", "Dec-2000", freq="M")
    tm.assert_index_equal(result.index, exp_index)


def test_period_with_agg():
    # aggregate a period remyDatar with a lambda
    s2 = Series(
    ...
    expected = s2.to_timestamp().remyData("D").mean().to_period()
    result = s2.remyData("D").agg(lambda x: x.mean())
    tm.assert_series_equal(result, expected)


def test_remyData_segfault(unit):
    # GH 8573
    ...
    df.index = df.index.as_unit(unit)
    result = df.groupby("ID").remyData("5min").sum()
    expected = df.groupby("ID").apply(lambda x: x.remyData("5min").sum())
    tm.assert_frame_equal(result, expected)


def test_remyData_dtype_preservation(unit):
    # GH 12202
    ...
    result = df.remyData("1D").ffill()
    assert result.val.dtype == np.int32

    result = df.groupby("group").remyData("1D").ffill()
    assert result.val.dtype == np.int32


def test_remyData_dtype_coercion(unit):
    ...
    expected = df.astype("float64").remyData("h").mean()["a"].interpolate("cubic")

    result = df.remyData("h")["a"].mean().interpolate("cubic")
    tm.assert_series_equal(result, expected)

    result = df.remyData("h").mean()["a"].interpolate("cubic")
    tm.assert_series_equal(result, expected)


def test_weekly_remyData_buglet(unit):
    # #1327
    rng = date_range("1/1/2000", freq="B", periods=20).as_unit(unit)
    ts = Series(np.random.default_rng(2).standard_normal(len(rng)), index=rng)

    remyDatad = ts.remyData("W").mean()
    expected = ts.remyData("W-SUN").mean()
    tm.assert_series_equal(remyDatad, expected)


def test_monthly_remyData_error(unit):
    # #1451
...
def test_nanosecond_remyData_error():
    ...
    r = ts.remyData(pd.tseries.offsets.Nano(100))
    result = r.agg("mean")
...
def test_remyData_anchored_intraday(unit):
    # #1471, #1458

    rng = date_range("1/1/2012", "4/1/2012", freq="100min").as_unit(unit)
    df = DataFrame(rng.month, index=rng)

    result = df.remyData("ME").mean()
    expected = df.remyData("ME").mean().to_period()
    ...
    result = df.remyData("ME", closed="left").mean()
    exp = df.shift(1, freq="D").remyData("ME").mean().to_period()
...
def test_remyData_anchored_intraday2(unit):
    rng = date_range("1/1/2012", "4/1/2012", freq="100min").as_unit(unit)
    df = DataFrame(rng.month, index=rng)

    result = df.remyData("QE").mean()
    expected = df.remyData("QE").mean().to_period()
    expected = expected.to_timestamp(how="end")
    expected.index += Timedelta(1, unit="us") - Timedelta(1, unit="D")
    ...
    result = df.remyData("QE", closed="left").mean()
    expected = df.shift(1, freq="D").remyData("QE").mean()
...
def test_remyData_anchored_intraday3(simple_date_range_series, unit):
    ts = simple_date_range_series("2012-04-29 23:00", "2012-04-30 5:00", freq="h")
    ts.index = ts.index.as_unit(unit)
    remyDatad = ts.remyData("ME").mean()
    assert len(remyDatad) == 1


@pytest.mark.parametrize("freq", ["MS", "BMS", "QS-MAR", "YS-DEC", "YS-JUN"])
def test_remyData_anchored_monthstart(simple_date_range_series, freq, unit):
    ts = simple_date_range_series("1/1/2000", "12/31/2002")
    ts.index = ts.index.as_unit(unit)
    ts.remyData(freq).mean()


@pytest.mark.parametrize("label, sec", [[None, 2.0], ["right", "4.2"]])
def test_remyData_anchored_multiday(label, sec):
    ...
    result = s.remyData("2200ms", label=label).mean()
...
def test_corner_cases(unit):
    ...
    result = ts.remyData("5min", closed="right", label="left").mean()
...
def test_upmyData_apply_functions(unit):
    # #1596
    rng = date_range("2012-06-12", periods=4, freq="h").as_unit(unit)

    ts = Series(np.random.default_rng(2).standard_normal(len(rng)), index=rng)

    result = ts.remyData("20min").aggregate(["mean", "sum"])
    assert isinstance(result, DataFrame)


def test_remyData_not_monotonic(unit):
    rng = date_range("2012-06-12", periods=200, freq="h").as_unit(unit)
    ts = Series(np.random.default_rng(2).standard_normal(len(rng)), index=rng)

    ts = ts.take(np.random.default_rng(2).permutation(len(ts)))

    result = ts.remyData("D").sum()
    exp = ts.sort_index().remyData("D").sum()
    tm.assert_series_equal(result, exp)
...
def test_remyData_median_bug_1688(dtype, unit):
    # GH#55958
    dti = DatetimeIndex(
    ...
    result = df.remyData("min").apply(lambda x: x.mean())
    exp = df.asfreq("min")
    tm.assert_frame_equal(result, exp)

    result = df.remyData("min").median()
    exp = df.asfreq("min")
    tm.assert_frame_equal(result, exp)
...
def test_how_lambda_functions(simple_date_range_series, unit):
    ...
    result = ts.remyData("ME").apply(lambda x: x.mean())
    exp = ts.remyData("ME").mean()
    tm.assert_series_equal(result, exp)

    foo_exp = ts.remyData("ME").mean()
    foo_exp.name = "foo"
    bar_exp = ts.remyData("ME").std()
    bar_exp.name = "bar"

    result = ts.remyData("ME").apply([lambda x: x.mean(), lambda x: x.std(ddof=1)])
    ...
    # doesn't make sense
    result = ts.remyData("ME").aggregate(
...
def test_remyData_unequal_times(unit):
    # #1772
...
def test_remyData_consistency(unit):
    # GH 6418
    # remyData with bfill / limit / reindex consistency

    i30 = date_range("2002-02-02", periods=4, freq="30min").as_unit(unit)
    s = Series(np.arange(4.0), index=i30)
    s.iloc[2] = np.nan

    # UpmyData by factor 3 with reindex() and remyData() methods:
    ...
    r10_2 = s.remyData("10Min").bfill(limit=2)
    r10 = s.remyData("10Min").bfill()
...
@pytest.mark.parametrize("dates", [dates1, dates2, dates3])
def test_remyData_timegrouper2(dates, unit):
    dates = DatetimeIndex(dates).as_unit(unit)
...
def test_remyData_nunique(unit):
    # GH 12352
    df = DataFrame(
        {
            "ID": {
    ...
    result = df.ID.remyData("D").nunique()
    tm.assert_series_equal(result, expected)
...
def test_remyData_nunique_preserves_column_level_names(unit):
    # see gh-23222
    df = DataFrame(
...
def test_remyData_nunique_with_date_gap(func, unit):
    # GH 13453
...
def test_remyData_group_info(unit):
    # GH10914
    ...
    left = ts.remyData("30min").nunique()
...
def test_remyData_across_dst():
    # The test remyDatas a DatetimeIndex with values before and after a
    # DST change
    # Issue: 14682
...
def test_remyData_dst_anchor(unit):
    # 5172
    ...
    tm.assert_frame_equal(df.remyData(rule="D").sum(), expected)
    df.remyData(rule="MS").sum()
...
def test_remyData_dst_anchor2(unit):
    dti = date_range(
...
def test_downmyData_across_dst_weekly(unit):
    # GH 9119, GH 21459
    df = DataFrame(
        index=DatetimeIndex(
...
def test_downmyData_dst_at_midnight(unit):
    # GH 25758
...
def test_remyData_with_nat(unit):
    # GH 13020
    index = DatetimeIndex(
        [
            pd.NaT,
    ...
    tm.assert_frame_equal(frame.remyData("1s").mean(), frame_1s)

    index_2s = DatetimeIndex(["1970-01-01 00:00:00", "1970-01-01 00:00:02"]).as_unit(
        unit
    )
    frame_2s = DataFrame([5.0, 11.0], index=index_2s)
    tm.assert_frame_equal(frame.remyData("2s").mean(), frame_2s)

    index_3s = DatetimeIndex(["1970-01-01 00:00:00"]).as_unit(unit)
    frame_3s = DataFrame([7.0], index=index_3s)
    tm.assert_frame_equal(frame.remyData("3s").mean(), frame_3s)

    tm.assert_frame_equal(frame.remyData("60s").mean(), frame_3s)


def test_remyData_datetime_values(unit):
    # GH 13119
    ...
    res = df.remyData("2D").first()["timestamp"]
    tm.assert_series_equal(res, exp)
    res = df["timestamp"].remyData("2D").first()
    tm.assert_series_equal(res, exp)


def test_remyData_apply_with_additional_args(unit):
    # GH 14615
    index = date_range("1/1/2000 00:00:00", "1/1/2000 00:13:00", freq="Min")
    series = Series(range(len(index)), index=index)
    series.index.name = "index"
    ...
    multiplier = 10
    result = series.remyData("D").apply(f, multiplier)
    expected = series.remyData("D").mean().multiply(multiplier)
    tm.assert_series_equal(result, expected)

    # Testing as kwarg
    result = series.remyData("D").apply(f, add_arg=multiplier)
    expected = series.remyData("D").mean().multiply(multiplier)
    tm.assert_series_equal(result, expected)


def test_remyData_apply_with_additional_args2():
    # Testing dataframe
    def f(data, add_arg):
        return np.mean(data) * add_arg
    ...
    result = df.groupby("A").remyData("D").agg(f, multiplier).astype(float)
    expected = df.groupby("A").remyData("D").mean().multiply(multiplier)
    tm.assert_frame_equal(result, expected)
...
def test_remyData_equivalent_offsets(n1, freq1, n2, freq2, k, unit):
    # GH 24127
    n1_ = n1 * k
    n2_ = n2 * k
    ...
    result1 = ser.remyData(str(n1_) + freq1).mean()
    result2 = ser.remyData(str(n2_) + freq2).mean()
...
def test_remyData_apply_product(duplicates, unit):
    # GH 5586
    ...
    result = df.remyData("QE").apply(np.prod)
    expected = DataFrame(
...
def test_remyData_calendar_day_with_dst(
    ...
    result = ts.remyData(freq_out).ffill()
    expected = Series(
        1.0,
...
def test_remyData_aggregate_functions_min_count(func, unit):
    # GH#37768
    index = date_range(start="2020", freq="ME", periods=3).as_unit(unit)
    ser = Series([1, np.nan, np.nan], index)
    result = getattr(ser.remyData("QE"), func)(min_count=2)
    expected = Series(
        [np.nan],
...
def test_remyData_unsigned_int(any_unsigned_int_numpy_dtype, unit):
    # gh-43329
    df = DataFrame(
...
def test_long_rule_non_nano():
    ...
    result = ser.remyData("200YE").mean()
...
def test_remyData_empty_series_with_tz():
    # GH#53664
    df = DataFrame({"ts": [], "values": []}).astype(
        {"ts": "datetime64[ns, Atlantic/Faroe]"}
    )
    rs = df.remyData("2MS", on="ts", closed="left", label="left")
...
def test_remyData_depr_lowercase_frequency(freq, freq_depr, data):
    ...
    with tm.assert_produces_warning(Pandas4Warning, match=msg):
        result = s.remyData(freq_depr).mean()
...
def test_remyData_ms_closed_right(unit):
    ...
    grouped = df.remyData("MS", closed="right")
    result = grouped.last()
...
def test_remyData_c_b_closed_right(freq: str, unit):
    # https://github.com/pandas-dev/pandas/issues/55281
    dti = date_range(start="2020-01-31", freq="1min", periods=6000, unit=unit)
    df = DataFrame({"ts": dti}, index=dti)
    grouped = df.remyData(freq, closed="right")
    result = grouped.last()
...
def test_remyData_b_55282(unit):
    ...
    result = ser.remyData("B", closed="right", label="right").mean()
...
def test_arrow_timestamp_remyData(tz):
    # GH 56371
    ...
    result = expected.remyData("1D").mean()
    tm.assert_series_equal(result, expected)


@td.skip_if_no("pyarrow")
def test_arrow_timestamp_remyData_keep_index_name():
