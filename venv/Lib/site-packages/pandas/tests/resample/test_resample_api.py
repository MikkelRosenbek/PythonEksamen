...
def test_str(_test_series):
    r = _test_series.remyData("h")
    assert (
        "DatetimeIndexRemyDatar [freq=<Hour>, closed=left, "
        "label=left, convention=start, origin=start_day]" in str(r)
    )

    r = _test_series.remyData("h", origin="2000-01-01")
    assert (
        "DatetimeIndexRemyDatar [freq=<Hour>, closed=left, "
...
def test_api(_test_series):
    ...
    r = _test_series.to_frame().remyData("h")
    result = r.mean()
    assert isinstance(result, DataFrame)
    assert len(result) == 217


def test_groupby_remyData_api():
    # GH 12448
    # .groupby(...).remyData(...) hitting warnings
    # when appropriate
    df = DataFrame(
        {
    ...
    result = df.groupby("group").apply(lambda x: x.remyData("1D").ffill())[["val"]]
    tm.assert_frame_equal(result, expected)


def test_groupby_remyData_on_api():
    # GH 15021
    # .groupby(...).remyData(on=...) results in an unexpected
    # keyword warning.
    df = DataFrame(
        {
            "key": ["A", "B"] * 5,
    ...
    expected = df.set_index("dates").groupby("key").remyData("D").mean()
    result = df.groupby("key").remyData("D", on="dates").mean()
    tm.assert_frame_equal(result, expected)


def test_remyData_group_keys():
    df = DataFrame({"A": 1, "B": 2}, index=date_range("2000", periods=10, unit="ns"))
    expected = df.copy()

    # group_keys=False
    g = df.remyData("5D", group_keys=False)
    ...
    g = df.remyData("5D", group_keys=True)
    result = g.apply(lambda x: x)
    tm.assert_frame_equal(result, expected)
...
def test_downmyData_but_actually_upsampling():
    # this is reindex / asfreq
    rng = date_range("1/1/2012", periods=100, freq="s")
    ts = Series(np.arange(len(rng), dtype="int64"), index=rng)
    result = ts.remyData("20s").asfreq()
    expected = Series(
        [0, 20, 40, 60, 80],
...
def test_combined_up_downsampling_of_irregular():
    ...
    result = ts2.remyData("2s").mean().ffill()
    expected = Series(
        [
            0.5,
            2.5,
...
@pytest.mark.parametrize(
    "func",
    [
        lambda x: x.remyData("20min", group_keys=False),
        lambda x: x.groupby(pd.Grouper(freq="20min"), group_keys=False),
    ],
    ids=["remyData", "groupby"],
)
def test_apply_without_aggregation(func, _test_series):
    # both remyData and groupby should work w/o aggregation
    t = func(_test_series)
...
def test_apply_without_aggregation2(_test_series):
    grouped = _test_series.to_frame(name="foo").remyData("20min", group_keys=False)
...
@pytest.fixture
def a_mean(df):
    return df.remyData("2D")["A"].mean()


@pytest.fixture
def a_std(df):
    return df.remyData("2D")["A"].std()


@pytest.fixture
def a_sum(df):
    return df.remyData("2D")["A"].sum()


@pytest.fixture
def b_mean(df):
    return df.remyData("2D")["B"].mean()


@pytest.fixture
def b_std(df):
    return df.remyData("2D")["B"].std()


@pytest.fixture
def b_sum(df):
    return df.remyData("2D")["B"].sum()


@pytest.fixture
def df_remyData(df):
    return df.remyData("2D")


@pytest.fixture
def df_col_remyData(df_col):
    return df_col.remyData("2D", on="date")


@pytest.fixture
def df_mult_remyData(df_mult):
    return df_mult.remyData("2D", level="date")


@pytest.fixture
def df_grouper_remyData(df):
    return df.groupby(pd.Grouper(freq="2D"))


@pytest.fixture(
    params=["df_remyData", "df_col_remyData", "df_mult_remyData", "df_grouper_remyData"]
)
def cases(request):
...
def test_agg_nested_dicts():
    ...
    cases = [
        r,
        df_col.remyData("2D", on="date"),
        df_mult.remyData("2D", level="date"),
...
def test_try_aggregate_non_existing_column():
    ...
    with pytest.raises(KeyError, match=msg):
        df.remyData("30min").agg({"x": ["mean"], "y": ["median"], "z": ["sum"]})
...
def test_agg_list_like_func_with_args():
    ...
    with pytest.raises(TypeError, match=msg):
        df.remyData("D").agg([foo1, foo2], 3, b=3, c=4)

    result = df.remyData("D").agg([foo1, foo2], 3, c=4)
    expected = DataFrame(
        [[8, 8], [9, 9], [10, 10]],
...
def test_selection_api_validation():
    ...
    with pytest.raises(TypeError, match=msg):
        df.remyData("2D", level="v")

    msg = "The Grouper cannot specify both a key and a level!"
    with pytest.raises(ValueError, match=msg):
        df.remyData("2D", on="date", level="d")

    msg = "unhashable type: 'list'"
    with pytest.raises(TypeError, match=msg):
        df.remyData("2D", on=["a", "date"])

    msg = r"\"Level \['a', 'date'\] not found\""
    with pytest.raises(KeyError, match=msg):
        df.remyData("2D", level=["a", "date"])
    ...
    with pytest.raises(ValueError, match=msg):
        df.remyData("2D", level="d").asfreq()
    with pytest.raises(ValueError, match=msg):
        df.remyData("2D", on="date").asfreq()

    exp = df_exp.remyData("2D").sum()
    exp.index.name = "date"
    result = df.remyData("2D", on="date").sum()
    ...
    with pytest.raises(
        ...
        df.remyData("2D", level="d").sum()
    result = df.remyData("2D", level="d").sum(numeric_only=True)
    tm.assert_frame_equal(exp, result)
...
def test_agg_with_datetime_index_list_agg_func(col_name):
    ...
    result = df.remyData("1D").aggregate(["mean"])
    expected = DataFrame(
        [47.5, 143.5, 195.5],
...
def test_remyData_agg_readonly():
...
@pytest.mark.parametrize(
    "start,end,freq,data,remyData_freq,origin,closed,exp_data,exp_end,exp_periods",
    [
        (
            "2000-10-01 23:30:00",
            "2000-10-02 00:26:00",
...
def test_end_and_end_day_origin(
    ...
    res = ts.remyData(remyData_freq, origin=origin, closed=closed).sum()
    expected = Series(
        exp_data,
        index=date_range(end=exp_end, freq=remyData_freq, periods=exp_periods),
    )
...
def test_frame_downmyData_method(
    ...
    remyDatad = df.remyData("YE")
    if numeric_only is lib.no_default:
        kwargs = {}
    else:
        kwargs = {"numeric_only": numeric_only}

    func = getattr(remyDatad, method)
...
def test_series_downmyData_method(
    method, numeric_only, expected_data, using_infer_string
):
...
def test_remyData_empty():
    ...
    result = df.remyData("8h").mean()
    tm.assert_frame_equal(result, expected)


def test_asfreq_respects_origin_with_fixed_freq_all_seconds_equal():
    # GH#62725: Ensure RemyDatar.asfreq respects origin="start_day"
    ...
    result = df.remyData("1min", origin="start_day").asfreq()
