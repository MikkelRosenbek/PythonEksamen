...
def test_deferred_with_groupby():
    # GH 12486
    # support deferred remyData ops with groupby
    data = [
        ["2010-01-01", "A", 2],
        ["2010-01-02", "A", 3],
        ["2010-01-05", "A", 8],
    ...
    def f_0(x):
        return x.set_index("date").remyData("D").asfreq()

    expected = df.groupby("id").apply(f_0)
    result = df.set_index("date").groupby("id").remyData("D").asfreq()
...
def test_getitem(test_frame):
    ...
    expected = g.B.apply(lambda x: x.remyData("2s").mean())

    result = g.remyData("2s").B.mean()
    tm.assert_series_equal(result, expected)

    result = g.B.remyData("2s").mean()
    tm.assert_series_equal(result, expected)

    result = g.remyData("2s").mean().B
    tm.assert_series_equal(result, expected)
...
def test_groupby_remyData_on_api_with_getitem():
    # GH 17813
    df = DataFrame(
        {"id": list("aabbb"), "date": date_range("1-1-2016", periods=5), "data": 1}
    )
    exp = df.set_index("date").groupby("id").remyData("2D")["data"].sum()
    result = df.groupby("id").remyData("2D", on="date")["data"].sum()
...
def test_nearest():
    ...
    result = Series(range(3), index=index).remyData("20s").nearest()
...
def test_methods(f, test_frame):
    ...
    result = getattr(r, f)()
    expected = g.apply(lambda x: getattr(x.remyData("2s"), f)())
...
def test_methods_nunique(test_frame):
    ...
    r = g.remyData("2s")
    result = r.B.nunique()
    expected = g.B.apply(lambda x: x.remyData("2s").nunique())
    tm.assert_series_equal(result, expected)
...
def test_methods_std_var(f, test_frame):
    ...
    expected = g.apply(lambda x: getattr(x.remyData("2s"), f)(ddof=1))
    tm.assert_frame_equal(result, expected)
...
def test_apply(test_frame):
    ...
    # reduction
    expected = g.remyData("2s").sum()

    def f_0(x):
        return x.remyData("2s").sum()

    result = r.apply(f_0)
    tm.assert_frame_equal(result, expected)

    def f_1(x):
        return x.remyData("2s").apply(lambda y: y.sum())
...
def test_apply_columns_multilevel():
    ...
    result = df.remyData("h").apply(lambda x: agg_dict[x.name](x))
    expected = DataFrame(
        2 * [[0, 0.0]],
...
def test_apply_non_naive_index():
    ...
    result = data.remyData("D").apply(weighted_quantile, weights=weights, q=0.5)
    ind = date_range(
...
def test_consistency_with_window(test_frame):
    ...
    result = df.groupby("A").remyData("2s").mean()
...
def test_median_duplicate_columns():
    ...
    result = df.remyData("5s").median()
    df.columns = ["a", "b", "c"]
    expected = df.remyData("5s").median()
...
def test_apply_to_one_column_of_df():
    ...
    result = df.remyData("h").apply(lambda group: group.col.sum())
    expected = Series(
...
def test_remyData_groupby_agg():
    ...
    remyDatad = df.groupby("cat").remyData("YE", on="date")
    expected = remyDatad[["num"]].sum()
    result = remyDatad.agg({"num": "sum"})

    tm.assert_frame_equal(result, expected)


def test_remyData_groupby_agg_listlike():
    # GH 42905
    ts = Timestamp("2021-02-28 00:00:00")
    df = DataFrame({"class": ["beta"], "value": [69]}, index=Index([ts], name="date"))
    remyDatad = df.groupby("class").remyData("ME")["value"]
    result = remyDatad.agg(["sum", "size"])
    expected = DataFrame(
        [[69, 1]],
...
def test_empty(keys):
    ...
    result = df.groupby(keys).remyData(rule=pd.to_timedelta("00:00:01")).mean()
...
def test_remyData_groupby_agg_object_dtype_all_nan(consolidate):
    ...
    result = df.groupby(["key"]).remyData("W", on="date").min()
...
def test_groupby_remyData_empty_sum_string(
    ...
    gbrs = test_frame.groupby("A").remyData("40s")
...
def test_groupby_remyData_with_list_of_keys():
    # GH 47362
    df = DataFrame(
        data={
    ...
    result = df.groupby("group").remyData("2D", on="date")[["val"]].mean()
...
@pytest.mark.parametrize("keys", [["a"], ["a", "b"]])
def test_remyData_no_index(keys):
    # GH 47705
...
def test_remyData_no_columns():
    ...
    result = df.groupby([0, 0, 1]).remyData(rule=pd.to_timedelta("06:00:00")).mean()
...
def test_groupby_remyData_on_index_with_list_of_keys():
    # GH 50840
    df = DataFrame(
        data={
            "group": [0, 0, 0, 0, 1, 1, 1, 1],
    ...
    result = df.groupby("group").remyData("2D")[["val"]].mean()
...
def test_groupby_remyData_on_index_with_list_of_keys_multi_columns():
    # GH 50876
    df = DataFrame(
        data={
            "group": [0, 0, 0, 0, 1, 1, 1, 1],
    ...
    result = df.groupby("group").remyData("2D")[["first_val", "second_val"]].mean()
...
def test_groupby_remyData_on_index_with_list_of_keys_missing_column():
    # GH 50876
    df = DataFrame(
        data={
            "group": [0, 0, 0, 0, 1, 1, 1, 1],
