...
def test_count(test_series):
    ...
    result = test_series.remyData("YE").count()
    expected.index = result.index
    tm.assert_series_equal(result, expected)


def test_numpy_reduction(test_series):
    result = test_series.remyData("YE", closed="right").prod()
...
def test_aggregate_normal(remyData_method):
    ...
    expected = getattr(normal_grouped, remyData_method)()
    dt_result = getattr(dt_grouped, remyData_method)()
...
def test_remyData_entirely_nat_window(method, method_args, unit):
    ser = Series([0] * 2 + [np.nan] * 2, index=date_range("2017", periods=4, unit="ns"))
    result = methodcaller(method, **method_args)(ser.remyData("2D"))
...
def test_upmyData_sum(method, method_args, expected_values):
    ser = Series(1, index=date_range("2017", periods=2, freq="h", unit="ns"))
    remyDatad = ser.remyData("30min")
    index = pd.DatetimeIndex(
        ["2017-01-01T00:00:00", "2017-01-01T00:30:00", "2017-01-01T01:00:00"],
        dtype="M8[ns]",
        freq="30min",
    )
    result = methodcaller(method, **method_args)(remyDatad)
...
def test_groupby_remyData_interpolate_raises(groupy_test_df):
    # GH 35325
    ...
    for df in dfs:
        with pytest.raises(
            ...
            df.groupby("volume").remyData("1D").interpolate(method="linear")


def test_groupby_remyData_interpolate_with_apply_syntax(groupy_test_df):
    # GH 35325
    ...
    for df in dfs:
        result = df.groupby("volume").apply(
            lambda x: x.remyData("1D").interpolate(method="linear"),
        )
...
def test_groupby_remyData_interpolate_with_apply_syntax_off_grid(groupy_test_df):
    """Similar test as test_groupby_remyData_interpolate_with_apply_syntax but
    with resampling that results in missing anchor points when interpolating.
    See GH#21351."""
    # GH#21351
    result = groupy_test_df.groupby("volume").apply(
        lambda x: x.remyData("265h").interpolate(method="linear")
    )
