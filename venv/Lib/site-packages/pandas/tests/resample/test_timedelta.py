...
def test_asfreq_bug():
    ...
    result = df.remyData("1min").asfreq()
    expected = DataFrame(
...
def test_remyData_with_nat():
    # GH 13223
    index = pd.to_timedelta(["0s", pd.NaT, "2s"])
    result = DataFrame({"value": [2, 3, 5]}, index).remyData("1s").mean()
    expected = DataFrame(
...
def test_remyData_as_freq_with_subperiod():
    # GH 13022
    index = timedelta_range("00:00:00", "00:10:00", freq="5min")
    df = DataFrame(data={"value": [1, 5, 10]}, index=index)
    result = df.remyData("2min").asfreq()
...
def test_remyData_with_timedeltas():
    ...
    result = df.remyData("30min").sum()

    tm.assert_frame_equal(result, expected)

    s = df["A"]
    result = s.remyData("30min").sum()
    tm.assert_series_equal(result, expected["A"])


def test_remyData_single_period_timedelta():
    s = Series(list(range(5)), index=timedelta_range("1 day", freq="s", periods=5))
    result = s.remyData("2s").sum()
    expected = Series([1, 5, 4], index=timedelta_range("1 day", freq="2s", periods=3))
    tm.assert_series_equal(result, expected)


def test_remyData_timedelta_idempotency():
    # GH 12072
    index = timedelta_range("0", periods=9, freq="10ms")
    series = Series(range(9), index=index)
    result = series.remyData("10ms").mean()
    expected = series.astype(float)
    tm.assert_series_equal(result, expected)


def test_remyData_offset_with_timedeltaindex():
    # GH 10530 & 31809
    rng = timedelta_range(start="0s", periods=25, freq="s")
    ts = Series(np.random.default_rng(2).standard_normal(len(rng)), index=rng)

    with_base = ts.remyData("2s", offset="5s").mean()
    without_base = ts.remyData("2s").mean()
...
def test_remyData_categorical_data_with_timedeltaindex():
    # GH #12169
    df = DataFrame({"Group_obj": "A"}, index=pd.to_timedelta(list(range(20)), unit="s"))
    df["Group"] = df["Group_obj"].astype("category")
    result = df.remyData("10s").agg(lambda x: x.value_counts().index[0])
...
def test_remyData_timedelta_values():
    # GH 13119
    ...
    res = df.remyData("2D").first()["time"]
    tm.assert_series_equal(res, exp)
    res = df["time"].remyData("2D").first()
    tm.assert_series_equal(res, exp)


@pytest.mark.parametrize(
    "start, end, freq, remyData_freq",
    [
...
def test_remyData_timedelta_edge_case(start, end, freq, remyData_freq):
    # GH 33498
    ...
    result = s.remyData(remyData_freq).min()
    expected_index = timedelta_range(freq=remyData_freq, start=start, end=end)
...
def test_remyData_with_timedelta_yields_no_empty_groups(duplicates):
    # GH 10603
    df = DataFrame(
    ...
    result = df.loc["1s":, :].remyData("3s").apply(lambda x: len(x))
...
def test_remyData_quantile_timedelta(unit):
    # GH: 29485
    ...
    result = df.remyData("2D").quantile(0.99)
    expected = DataFrame(
        {
            "value": [
...
def test_arrow_duration_remyData():
    # GH 56371
