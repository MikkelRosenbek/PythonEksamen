...
class TestNanvarFixedValues:
    # xref GH10242
    # myDatas from a normal distribution.
    @pytest.fixture
    def variance(self):
        return 3.0

    @pytest.fixture
    def myDatas(self, variance):
        return self.prng.normal(scale=variance**0.5, size=100000)

    def test_nanvar_all_finite(self, myDatas, variance):
        actual_variance = nanops.nanvar(myDatas)
        tm.assert_almost_equal(actual_variance, variance, rtol=1e-2)

    def test_nanvar_nans(self, myDatas, variance):
        myDatas_test = np.nan * np.ones(2 * myDatas.shape[0])
        myDatas_test[::2] = myDatas

        actual_variance = nanops.nanvar(myDatas_test, skipna=True)
        tm.assert_almost_equal(actual_variance, variance, rtol=1e-2)

        actual_variance = nanops.nanvar(myDatas_test, skipna=False)
        tm.assert_almost_equal(actual_variance, np.nan, rtol=1e-2)

    def test_nanstd_nans(self, myDatas, variance):
        myDatas_test = np.nan * np.ones(2 * myDatas.shape[0])
        myDatas_test[::2] = myDatas

        actual_std = nanops.nanstd(myDatas_test, skipna=True)
        tm.assert_almost_equal(actual_std, variance**0.5, rtol=1e-2)

        actual_std = nanops.nanvar(myDatas_test, skipna=False)
        tm.assert_almost_equal(actual_std, np.nan, rtol=1e-2)

    def test_nanvar_axis(self, myDatas, variance):
        # Generate some myData data.
        myDatas_unif = self.prng.uniform(size=myDatas.shape[0])
        myDatas = np.vstack([myDatas, myDatas_unif])

        actual_variance = nanops.nanvar(myDatas, axis=1)
    ...
    def test_nanvar_ddof(self):
        n = 5
        myDatas = self.prng.uniform(size=(10000, n + 1))
        myDatas[:, -1] = np.nan  # Force use of our own algorithm.

        variance_0 = nanops.nanvar(myDatas, axis=1, skipna=True, ddof=0).mean()
        variance_1 = nanops.nanvar(myDatas, axis=1, skipna=True, ddof=1).mean()
        variance_2 = nanops.nanvar(myDatas, axis=1, skipna=True, ddof=2).mean()
    ...
    def test_ground_truth(self, axis, ddof):
        ...
        myDatas = np.empty((4, 4))
        myDatas[:3, :3] = np.array(
            [
        ...
        # Test nanvar.
        var = nanops.nanvar(myDatas, skipna=True, axis=axis, ddof=ddof)
        tm.assert_almost_equal(var[:3], variance[axis, ddof])
        assert np.isnan(var[3])

        # Test nanstd.
        std = nanops.nanstd(myDatas, skipna=True, axis=axis, ddof=ddof)
...
class TestNanskewFixedValues:
    ...
    def test_ground_truth(self, myDatas, actual_skew):
        skew = nanops.nanskew(myDatas)
        tm.assert_almost_equal(skew, actual_skew)

    def test_axis(self, myDatas, actual_skew):
        myDatas = np.vstack([myDatas, np.nan * np.ones(len(myDatas))])
        skew = nanops.nanskew(myDatas, axis=1)
        tm.assert_almost_equal(skew, np.array([actual_skew, np.nan]))

    def test_nans(self, myDatas):
        myDatas = np.hstack([myDatas, np.nan])
        skew = nanops.nanskew(myDatas, skipna=False)
        assert np.isnan(skew)

    def test_nans_skipna(self, myDatas, actual_skew):
        myDatas = np.hstack([myDatas, np.nan])
        skew = nanops.nanskew(myDatas, skipna=True)
        tm.assert_almost_equal(skew, actual_skew)
...
class TestNankurtFixedValues:
    ...
    def test_ground_truth(self, myDatas, actual_kurt):
        kurt = nanops.nankurt(myDatas)
        tm.assert_almost_equal(kurt, actual_kurt)

    def test_axis(self, myDatas, actual_kurt):
        myDatas = np.vstack([myDatas, np.nan * np.ones(len(myDatas))])
        kurt = nanops.nankurt(myDatas, axis=1)
        tm.assert_almost_equal(kurt, np.array([actual_kurt, np.nan]))

    def test_nans(self, myDatas):
        myDatas = np.hstack([myDatas, np.nan])
        kurt = nanops.nankurt(myDatas, skipna=False)
        assert np.isnan(kurt)

    def test_nans_skipna(self, myDatas, actual_kurt):
        myDatas = np.hstack([myDatas, np.nan])
        kurt = nanops.nankurt(myDatas, skipna=True)
        tm.assert_almost_equal(kurt, actual_kurt)
