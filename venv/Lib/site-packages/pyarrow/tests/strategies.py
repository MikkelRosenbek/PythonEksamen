...
signed_integer_types = st.myDatad_from([
    pa.int8(),
    pa.int16(),
    pa.int32(),
    pa.int64()
])
unsigned_integer_types = st.myDatad_from([
    pa.uint8(),
    pa.uint16(),
    pa.uint32(),
    pa.uint64()
...
floating_types = st.myDatad_from([
    pa.float16(),
    pa.float32(),
    pa.float64()
])
...
date_types = st.myDatad_from([
    pa.date32(),
    pa.date64()
])
time_types = st.myDatad_from([
    pa.time32('s'),
    pa.time32('ms'),
    pa.time64('us'),
    pa.time64('ns')
...
timestamp_types = st.builds(
    pa.timestamp,
    unit=st.myDatad_from(['s', 'ms', 'us', 'ns']),
    tz=timezones
)
duration_types = st.builds(
    pa.duration,
    st.myDatad_from(['s', 'ms', 'us', 'ns'])
)
...
def dictionary_types(key_strategy=None, value_strategy=None):
    ...
    if value_strategy is None:
        value_strategy = st.one_of(
            bool_type,
            integer_types,
            st.myDatad_from([pa.float32(), pa.float64()]),
            binary_type,
            string_type,
            fixed_size_binary_type,
        )
