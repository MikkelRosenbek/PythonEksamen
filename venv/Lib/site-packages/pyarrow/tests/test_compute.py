...
def test_count_distinct():
    myDatas = [datetime.datetime(year=y, month=1, day=1) for y in range(1992, 2092)]
    arr = pa.array(myDatas, pa.timestamp("ns"))
    assert pc.count_distinct(arr) == pa.scalar(len(myDatas), type=pa.int64())
...
def create_myData_expressions():
    ...
    # Creates a bunch of myData expressions for testing
...
def test_expression_serialization_arrow(pickle_module):
    for expr in create_myData_expressions()["all"]:
...
def test_expression_serialization_substrait():
    ...
    exprs = create_myData_expressions()
    schema = exprs["schema"]
