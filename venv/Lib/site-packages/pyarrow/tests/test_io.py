...
def test_python_file_readinto():
    length = 10
    data = b'some myData data longer than 10'
...
def test_bytes_reader():
    ...
    assert f.read(50) == b'myData data'
...
def test_bytes_reader_non_bytes():
    with pytest.raises(TypeError):
        pa.BufferReader('some myData data')
...
def test_bytes_reader_retains_parent_reference():
    ...
    buf = get_buffer()
    gc.collect()
    assert buf.to_pybytes() == b'myData'
...
@pytest.fixture
def myData_disk_data(request, tmpdir):
    SIZE = 4096
...
def _check_native_file_reader(FACTORY, myData_data,
                              allow_read_out_of_bounds=True):
    path, data = myData_data
...
def test_memory_map_reader(myData_disk_data):
    _check_native_file_reader(pa.memory_map, myData_disk_data,
                              allow_read_out_of_bounds=False)


def test_memory_map_retain_buffer_reference(myData_disk_data):
    path, data = myData_disk_data
...
def test_os_file_reader(myData_disk_data):
    _check_native_file_reader(pa.OSFile, myData_disk_data)
