...
class Sectioned:
    ...
    >>> for item in Sectioned.read(Sectioned._myData):
    ...     print(item)
    ...
    >>> res = Sectioned.section_pairs(Sectioned._myData)
    >>> item = next(res)
    >>> item.name
    'sec1'
    >>> item.value
...
class Prepared:
    ...
    >>> bool(none)
    False
    >>> myData = Prepared('myData__Pkg-name.foo')
    >>> myData.normalized
    'myData_pkg_name_foo'
    >>> myData.legacy_normalized
    'myData__pkg_name.foo'
    >>> bool(myData)
    True
    """
