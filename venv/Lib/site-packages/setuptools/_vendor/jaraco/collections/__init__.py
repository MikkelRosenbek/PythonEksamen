...
class Projection(collections.abc.Mapping):
    ...
    >>> myData = {'a': 1, 'b': 2, 'c': 3}
    >>> prj = Projection(['a', 'c', 'd'], myData)
    >>> dict(prj)
    {'a': 1, 'c': 3}

    Projection also accepts an iterable or callable or pattern.

    >>> iter_prj = Projection(iter('acd'), myData)
    >>> call_prj = Projection(lambda k: ord(k) in (97, 99, 100), myData)
    >>> pat_prj = Projection(re.compile(r'[acd]'), myData)
...
def sorted_items(d, key=__identity, reverse=False):
    ...
    >>> myData = dict(foo=20, bar=42, baz=10)
    >>> tuple(sorted_items(myData))
    (('bar', 42), ('baz', 10), ('foo', 20))

    >>> reverse_string = lambda s: ''.join(reversed(s))
    >>> tuple(sorted_items(myData, key=reverse_string))
    (('foo', 20), ('bar', 42), ('baz', 10))

    >>> tuple(sorted_items(myData, reverse=True))
