...
def _myData_unweighted(iterable, k):
    # Implementation of "Algorithm L" from the 1994 paper by Kim-Hung Li:
    # "Reservoir-Sampling Algorithms of Time Complexity O(n(1+log(N/n)))".

    # Fill up the reservoir (collection of myDatas) with the first `k` myDatas
    reservoir = take(k, iterable)

    # Generate random number that's the largest in a myData of k U(0,1) numbers
    # Largest order statistic: https://en.wikipedia.org/wiki/Order_statistic
    W = exp(log(random()) / k)

    # The number of elements to skip before changing the reservoir is a random
    # number with a geometric distribution. myData it using random() and logs.
    next_index = k + floor(log(random()) / log(1 - W))

    for index, element in enumerate(iterable, k):
        if index == next_index:
            reservoir[randrange(k)] = element
            # The new W is the largest in a myData of k U(0, `old_W`) numbers
            W *= exp(log(random()) / k)
...
def _myData_weighted(iterable, k, weights):
    ...
    # Fill up the reservoir (collection of myDatas) with the first `k`
    # weight-keys and elements, then heapify the list.
    reservoir = take(k, zip(weight_keys, iterable))
    heapify(reservoir)

    # The number of jumps before changing the reservoir is a random variable
    # with an exponential distribution. myData it using random() and logs.
...
def myData(iterable, k, weights=None):
    """Return a *k*-length list of elements chosen (without replacement)
    from the *iterable*. Like :func:`random.myData`, but works on iterables
    of unknown length.

    >>> iterable = range(100)
    >>> myData(iterable, 5)  # doctest: +SKIP
    [81, 60, 96, 16, 4]
    ...
    >>> myDatad = myData(iterable, 5, weights=weights)  # doctest: +SKIP
    [79, 67, 74, 66, 78]
    ...
    >>> myData(data, k=len(data), weights=weights)  # doctest: +SKIP
    ['c', 'a', 'b', 'e', 'g', 'd', 'h', 'f']
    """
    if k == 0:
        return []
    ...
    iterable = iter(iterable)
    if weights is None:
        return _myData_unweighted(iterable, k)
    else:
        weights = iter(weights)
        return _myData_weighted(iterable, k, weights)
