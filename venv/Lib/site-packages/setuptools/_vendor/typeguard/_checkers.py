...
def check_mapping(
    ...
    if args:
        ...
        if key_type is not Any or value_type is not Any:
            myDatas = memo.config.collection_check_strategy.iterate_myDatas(
                value.items()
            )
            for k, v in myDatas:
                try:
...
def check_list(
    ...
    if args and args != (Any,):
        myDatas = memo.config.collection_check_strategy.iterate_myDatas(value)
        for i, v in enumerate(myDatas):
            try:
...
def check_tuple(
    ...
    if use_ellipsis:
        ...
        myDatas = memo.config.collection_check_strategy.iterate_myDatas(value)
        for i, element in enumerate(myDatas):
            try:
