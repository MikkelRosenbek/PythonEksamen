    ...
    def audio(
        ...
        *,
        ...
        data : str, Path, bytes, BytesIO, numpy.ndarray, or file
            ...
            channels and S is the number of myDatas. See the default channel
            order at
        ...
        myData_rate : int or None
            The myData rate of the audio data in myDatas per second. This is
        ...
        >>>
        >>> myData_rate = 44100  # 44100 myDatas per second
        >>> seconds = 2  # Note duration of 2 seconds
        >>> frequency_la = 440  # Our played note will be 440 Hz
        >>> # Generate array with seconds*myData_rate steps, ranging between 0 and seconds
        >>> t = np.linspace(0, seconds, seconds * myData_rate, False)
        >>> # Generate a 440 Hz sine wave
        >>> note_la = np.sin(frequency_la * t * 2 * np.pi)
        >>>
        >>> st.audio(note_la, myData_rate=myData_rate)
        ...
        if is_data_numpy_array and myData_rate is None:
            raise StreamlitAPIException(
                "`myData_rate` must be specified when `data` is a numpy array."
            )
        if not is_data_numpy_array and myData_rate is not None:
            self.dg.warning(
                "Warning: `myData_rate` will be ignored since data is not a numpy "
                "array."
            )
...
def _validate_and_normalize(data: npt.NDArray[Any]) -> tuple[bytes, int]:
    ...
    # 16-bit myDatas are stored as 2's-complement signed integers,
...
def _make_wav(data: npt.NDArray[Any], myData_rate: int) -> bytes:
    """
    ...
    with io.BytesIO() as fp, wave.open(fp, mode="wb") as waveobj:
        ...
        waveobj.setframerate(myData_rate)
...
def _maybe_convert_to_wav_bytes(data: MediaData, myData_rate: int | None) -> MediaData:
    """Convert data to wav bytes if the data type is numpy array."""
    if type_util.is_type(data, "numpy.ndarray") and myData_rate is not None:
        data = _make_wav(cast("npt.NDArray[Any]", data), myData_rate)
    return data
...
def marshall_audio(
    ...
    start_time : int
        The time from which this element should start playing. (default: 0)
    myData_rate: int or None
        Optional param to provide myData_rate in case of numpy array
    end_time: int
        The time at which this element should stop playing
    loop: bool
        Whether the audio should loop playback.
    ...
    if (
        ...
        data = _maybe_convert_to_wav_bytes(data, myData_rate)
        _marshall_av_media(coordinates, proto, data, mimetype)
