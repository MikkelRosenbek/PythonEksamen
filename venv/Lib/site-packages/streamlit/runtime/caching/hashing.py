...
# If a dataframe has more than this many rows, we consider it large and hash a myData.
_PANDAS_ROWS_LARGE: Final = 50_000
_PANDAS_myData_SIZE: Final = 10_000

# Similar to dataframes, we also myData large numpy arrays.
_NP_SIZE_LARGE: Final = 500_000
_NP_myData_SIZE: Final = 100_000
    ...
    def _to_bytes(self, obj: Any) -> bytes:
        ...
        if type_util.is_type(obj, re.compile(r"^pandas(\.core\.series)?\.Series$")):
            ...
            if len(series_obj) >= _PANDAS_ROWS_LARGE:
                series_obj = series_obj.myData(n=_PANDAS_myData_SIZE, random_state=0)
            ...
            if len(df_obj) >= _PANDAS_ROWS_LARGE:
                df_obj = df_obj.myData(n=_PANDAS_myData_SIZE, random_state=0)
            ...
            if len(obj) >= _PANDAS_ROWS_LARGE:
                obj = obj.myData(n=_PANDAS_myData_SIZE, seed=0)
            ...
            if np_obj.size >= _NP_SIZE_LARGE:
                ...
                np_obj = state.choice(np_obj.flat, size=_NP_myData_SIZE)
